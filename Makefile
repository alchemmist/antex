...
ANTEX_PACKAGE_COMMIT := $(subst +dirty,,$(ANTEX_BUILD_COMMIT))
...
build-linux:
	ANTEX_BUILD_COMMIT="$(ANTEX_PACKAGE_COMMIT)" $(BAZEL) build $(ANTEX_BAZEL_BUILD_FLAGS) --stamp --workspace_status_command="$(PYTHON) scripts/workspace-status.py" -c opt --platforms=//:local_linux //antex-rs/cli:antex //antex-rs/code-mode-host:antex-code-mode-host //antex-rs/voice-host:antex-voice-host //third_party/voice:native_runtime //antex-rs/bwrap:bwrap

.PHONY: package-macos-arm64 package-linux
package-macos-arm64:
	$(MAKE) build-macos-arm64 ANTEX_BUILD_COMMIT="$(ANTEX_PACKAGE_COMMIT)"
	ANTEX_BUILD_COMMIT="$(ANTEX_PACKAGE_COMMIT)" $(BAZEL) build $(ANTEX_BAZEL_BUILD_FLAGS) --stamp --workspace_status_command="$(PYTHON) scripts/workspace-status.py" -c opt --config=macos-arm64 //antex-rs/voice-host:antex-voice-host //third_party/voice:native_runtime
	ANTEX_REPO_ROOT="$(CURDIR)" $(PYTHON) scripts/package-fork.py --target aarch64-apple-darwin --commit "$(ANTEX_PACKAGE_COMMIT)" --output "$(ANTEX_PACKAGE_DIR)" $(ANTEX_PACKAGE_FLAGS)

package-linux: build-linux
	ANTEX_REPO_ROOT="$(CURDIR)" $(PYTHON) scripts/package-fork.py --target x86_64-unknown-linux-gnu --commit "$(ANTEX_PACKAGE_COMMIT)" --output "$(ANTEX_PACKAGE_DIR)" $(ANTEX_PACKAGE_FLAGS)
...
install-local:
	...
	$(PYTHON) scripts/smoke-voice-package.py "$$staging/package/bin/antex" "$(ANTEX_PACKAGE_COMMIT)"; \
