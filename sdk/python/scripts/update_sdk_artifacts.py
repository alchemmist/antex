...
def schema_bundle_path(schema_dir: Path) -> Path:
    ...
    return schema_dir / "antex_app_server_protocol.v2.schemas.json"
...
def stage_python_sdk_package(
    ...
    if codex_version is not None:
        runtime_version = normalize_antex_version(codex_version)
    ...
    if len(runtime_versions) != 1:
        raise RuntimeError("Expected exactly one pinned Antex runtime dependency")
    requirements = runpy.run_path(sdk_root() / "src/antex_sdk/_runtime_requirements.py")
    try:
...
def stage_python_runtime_package(
    ...
    if package_source.is_dir():
        ...
        if source.is_relative_to(destination) or destination.is_relative_to(source):
            raise RuntimeError("Antex package and runtime staging directories must not overlap")
        for path in package_source.rglob("*"):
            if path.is_symlink() or not (path.is_file() or path.is_dir()):
                raise RuntimeError(f"Expected a regular Antex package entry: {path}")
...
def _validate_codex_package_layout(package_dir: Path, package_source: Path) -> None:
    ...
    if missing_entries:
        ...
        raise RuntimeError(f"Missing Antex package layout entries in {package_source}: {missing}")
...
def _render_thread_block(turn_fields: list[PublicFieldSpec], *, is_async: bool = False) -> str:
    ...
    client = "self._antex._client" if is_async else "self._client"
    handle_type = "AsyncTurnHandle" if is_async else "TurnHandle"
    handle_owner = "self._antex" if is_async else "self._client"
    lines = [
        ...
        *(["        await self._antex._ensure_initialized()"] if is_async else []),
        _approval_mode_assignment_line("_approval_mode_override_settings"),
...
def build_parser() -> argparse.ArgumentParser:
    ...
    stage_runtime_parser.add_argument(
        "package_source",
        type=Path,
        help="Path to a Antex package directory or .tar.gz archive for this platform.",
    )
    stage_runtime_parser.add_argument(
        "--codex-version",
        required=True,
...
def run_command(args: argparse.Namespace, ops: CliOps) -> None:
    if args.command == "generate-types":
        ...
        if schema_dir is None:
            ...
            schema_dir = sdk_root() / pyproject["tool"]["antex"]["codegen"]["schema-dir"]
        ...
        ops.stage_python_sdk_package(
            ...
            normalize_antex_version(args.codex_version) if args.codex_version is not None else None,
        )
    elif args.command == "stage-runtime":
        ops.stage_python_runtime_package(
            args.staging_dir,
            normalize_antex_version(args.codex_version),
