...
from antex_sdk._runtime_requirements import CheckoutCapabilities
from antex_sdk.client import AntexClient, _params_dict
from antex_sdk.errors import AntexError
...
from antex_sdk.models import InitializeResponse, JsonObject, Notification, UnknownNotification
...
) -> tuple[AntexClient, list[tuple[str, JsonObject | None]]]:
    client = AntexClient()
...
def test_new_options_reject_unsupported_runtime_before_sending(
    ...
    client, requests = _initialized_client(monkeypatch, {"userAgent": f"antex-cli/{version}"})

    with pytest.raises(AntexError, match=r"Antex CLI 0\.151\.0 or newer"):
...
@pytest.mark.parametrize(
    "metadata",
    [
        {"userAgent": "antex-cli/0.151.0 (Linux)"},
        {"userAgent": "codex-cli 0.153.0"},
        {"userAgent": "antex-cli/0.154.0-alpha.1"},
        {"userAgent": "antex-cli/0.154.0-alpha.1.2"},
        {"userAgent": "antex-cli/0.151.0.post1"},
...
def test_unversioned_checkout_probes_and_caches_its_own_schema(
    ...
    client, requests = _initialized_client(monkeypatch, {"userAgent": "antex-cli/0.0.0"})
    ...
    monkeypatch.setattr("antex_sdk._runtime_requirements.subprocess.run", generate_schema)
    for method, params in (
        ...
        if supports_options:
            ...
            with pytest.raises(AntexError, match="checkout does not support"):
                client.request(method, params, response_model=InitializeResponse)
...
def test_unversioned_custom_launch_requires_verifiable_capabilities(
    ...
    client, requests = _initialized_client(monkeypatch, {"userAgent": "antex-cli/0.0.0"})
    with pytest.raises(AntexError, match="Cannot verify an unversioned CLI"):
        client.request(
...
@pytest.mark.parametrize("metadata", [{}, {"userAgent": "antex-cli/0.147.0"}])
...
def test_new_options_require_fresh_initialize_metadata_after_close(
    ...
    client, requests = _initialized_client(monkeypatch, {"userAgent": "antex-cli/0.153.0"})
    client.close()

    with pytest.raises(AntexError, match="reported version is 'unknown'"):
...
def test_decoded_notifications_match_the_declared_payload_type(method, params, expected) -> None:
    event = AntexClient()._coerce_notification(method, params)
