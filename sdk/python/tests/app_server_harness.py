...
class AppServerHarness:
    ...
    def app_server_config(self) -> AntexConfig:
        ...
        binary_name = "antex.exe" if os.name == "nt" else "antex"
        debug_binary = Path(__file__).resolve().parents[3] / "antex-rs/target/debug" / binary_name
        antex_bin = os.environ.get("ANTEX_EXEC_PATH")
        if antex_bin is None and debug_binary.is_file():
            antex_bin = str(debug_binary)
        return AntexConfig(
            antex_bin=antex_bin,
