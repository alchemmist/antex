...
from antex_sdk.api import AsyncThread, AsyncTurnHandle, Thread, TurnHandle
from antex_sdk.async_client import AsyncAntexClient
from antex_sdk.client import AntexClient
from antex_sdk.errors import TransportClosedError
...
def test_events_or_failure_before_turn_start_returns(monkeypatch, async_api, low_level, completed):
    ...
    client = codex._client._sync if async_api else AntexClient()
...
def test_cancelled_queued_start_does_not_send_a_request(monkeypatch):
    with ThreadPoolExecutor(max_workers=1) as executor:
        monkeypatch.setattr("antex_sdk.async_client._TURN_START_EXECUTOR", executor)
