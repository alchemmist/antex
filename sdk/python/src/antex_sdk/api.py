...
class AsyncThread:
    ...
    async def turn(
        ...
        *,
        ...
        turn, subscription = await self._antex._client._start_turn(
            self.id, wire_input, params=params, for_handle=True
        )
        return AsyncTurnHandle(self._antex, self.id, turn.turn.id, _subscription=subscription)
...
class TurnHandle:
    ...
    def __init__(
        self, _client: AntexClient, thread_id: str, id: str, *, _subscription=None
    ) -> None:
...
class AsyncTurnHandle:
    ...
    def __init__(self, _antex: AsyncAntex, thread_id: str, id: str, *, _subscription=None) -> None:
        self._antex, self.thread_id, self.id = _antex, thread_id, id
        if _subscription is None:
            self.__post_init__()
        else:
            self._subscription = _subscription
    ...
    def __post_init__(self) -> None:
        self._subscription = self._antex._client._subscribe_turn_notifications(self.id)
