...
impl RealtimeWebrtcSession {
    ...
    pub fn is_supported() -> bool {
        ...
        )) && antex_install_context::InstallContext::current()
            .package_layout
            .as_ref()
    ...
    pub fn start(abort: AbortRegistration) -> Result<StartedRealtimeWebrtcSession> {
        let package = antex_install_context::InstallContext::current()
            .package_layout
            .clone()
            .ok_or_else(|| anyhow::anyhow!("voice package unavailable"))?;
        let build_commit = antex_build_info::BuildInfo::get().build_commit().to_owned();
...
fn package_has_runtime(package: &std::path::Path) -> bool {
    let voice = package.join("antex-resources/voice");
    let (helper, runtime) = if cfg!(target_os = "macos") {
        ("bin/antex-voice-host", "lib/libgstreamer-1.0.0.dylib")
    } else if cfg!(windows) {
        ("bin/antex-voice-host.exe", "bin/gstreamer-1.0-0.dll")
    } else {
        ("bin/antex-voice-host", "lib/libgstreamer-1.0.so.0")
    };
