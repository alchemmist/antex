...
use antex_realtime_webrtc::AudioState;
...
const APP: &str = if cfg!(windows) { "antex.exe" } else { "antex" };
...
pub fn package(test: &str) -> Result<Option<PathBuf>> {
    ...
    if source.file_name().is_some_and(|name| name == APP) {
        antex_build_info::BuildInfo::initialize(BUILD_COMMIT);
        return Ok(Some(
            source
                .parent()
                .context("bin")?
    ...
    fs::create_dir_all(root.join("antex-resources/voice/bin"))?;
    fs::write(root.join("antex-package.json"), "{}")?;
    fs::copy(&source, root.join("bin").join(APP))?;
    fs::copy(&source, root.join("antex-resources/voice/bin").join(HELPER))?;
