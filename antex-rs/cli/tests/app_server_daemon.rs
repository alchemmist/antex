...
impl TestDaemon {
    fn new() -> Result<Self> {
        ...
        let codex = antex_utils_cargo_bin::cargo_bin("antex")?;
        let antex_source = std::fs::canonicalize(&codex)?;
        ...
        std::fs::hard_link(&antex_source, &managed)
            .or_else(|_| std::fs::copy(&antex_source, managed).map(|_| ()))?;
    ...
    fn command(&self) -> Command {
        ...
        command.env("ANTEX_HOME", self.home.path());
        command
    }
...
fn manual_update_rejects_an_unowned_installation() -> Result<()> {
    ...
    std::fs::remove_file(
        daemon
            .home
            .path()
            .join("packages/standalone/current/bin/antex"),
    )?;
...
fn packaged_daemon_launch(action: &str, initial: InitialDaemon) -> Result<()> {
    ...
    for directory in ["bin", "antex-path", "antex-resources"] {
        std::fs::create_dir_all(package.join(directory))?;
    }
    std::fs::copy(&daemon.codex, package.join("bin/antex"))?;
    daemon.codex = package.join("bin/antex");
    for helper in [
    ...
    std::fs::write(
        package.join("antex-package.json"),
        serde_json::to_vec(&serde_json::json!({
            "version": "0.0.0", "target": target, "entrypoint": "bin/antex"
        }))?,
    )?;
    ...
    if action == "start" {
        ...
        std::fs::remove_file(package.join("bin/antex-code-mode-host"))?;
        let invalid = daemon
            .command()
        ...
        std::fs::write(
            package.join("bin/antex-code-mode-host"),
        ...
        assert_eq!(
            std::fs::read(current.join("bin/antex-code-mode-host"))?,
            b"replacement helper"
        );
        if initial == InitialDaemon::Missing {
            assert_eq!(
                std::fs::read(original.join("bin/antex-code-mode-host"))?,
                b"runtime fixture"
            );
        }
        assert_eq!(daemon.lifecycle("start")?["status"], "alreadyRunning");
