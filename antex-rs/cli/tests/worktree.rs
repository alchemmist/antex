...
async fn interactive_worktree_start_and_fork_bind_owner_before_turn() -> anyhow::Result<()> {
    ...
    let root = antex_utils_absolute_path::AbsolutePathBuf::from_absolute_path(root.path())?
        .canonicalize()?
        .into_path_buf();
...
"#,
    ...
    let program = antex_utils_cargo_bin::cargo_bin("antex")?;
    let mut env: HashMap<String, String> = std::env::vars().collect();
    env.insert("ANTEX_HOME".into(), home.display().to_string());
    env.insert("ANTEX_SQLITE_HOME".into(), home.display().to_string());
    ...
    for (fork, explicit_cd, analytics, auth_failure) in [
        ...
        if auth_failure {
            ...
            fs::write(source.join(".antex/config.toml"), "")?;
        }
        ...
        if explicit_cd {
            args.extend(["--cd".into(), source.join(".antex").display().to_string()]);
        }
        if analytics {
        ...
        let renamed = if !fork && matches!(observed, Ok(Ok(_))) {
            tokio::time::timeout(Duration::from_secs(/*secs*/ 10), async {
                ...
                let thread_id = antex_protocol::ThreadId::from_string(
                    metadata["ownerThreadId"].as_str().context("owner id")?,
                )?;
                loop {
                    if antex_rollout::find_thread_name_by_id(&home, &thread_id)
                        .await?
                        .as_deref()
                        == Some("managed-source")
                    {
        ...
        let context = body["input"]
            ...
            .join("\n");
        if explicit_cd {
            let expected = format!("{checkout}/.antex").replace('\\', "/");
        ...
        if previous.is_empty() {
            ...
            let sqlite = antex_state::SqliteConfig::from_sqlite_home(
                antex_utils_absolute_path::AbsolutePathBuf::from_absolute_path(&home)?,
            );
            ...
            Mock::given(wiremock::matchers::path("/source/backend-api/wham/config/bundle"))
                .respond_with(ResponseTemplate::new(200).set_body_json(serde_json::json!({
                    "config_toml": {"enterprise_managed": [{"id":"distrust", "name":"distrust",
                        "contents": format!("[projects.{}]\ntrust_level = \"untrusted\"\n", serde_json::to_string(&antex_config::loader::project_trust_key(Path::new(&checkout)))?)}]}
            ...
            Mock::given(wiremock::matchers::path("/source/backend-api/wham/config/bundle"))
                .respond_with(ResponseTemplate::new(200).set_body_json(serde_json::json!({
                    "config_toml": {"enterprise_managed": [{"id":"distrust-source", "name":"distrust-source",
                        "contents": format!("[projects.{}]\ntrust_level = \"untrusted\"\n", serde_json::to_string(&antex_config::loader::project_trust_key(&source.join(".antex")))?)}]}
                }))).mount(&server).await;
            let output = rejected_start(
                ...
                &[
                    ...
                    "--no-alt-screen".into(),
                    "--cd".into(),
                    source.join(".antex").display().to_string(),
                ],
                &launcher,
                &env,
                "cannot create a checkout from an explicitly untrusted source",
    ...
    fs::write(source.join(".antex/config.toml"), "not = [valid toml")?;
    git(
        &source,
        &[
            "commit",
