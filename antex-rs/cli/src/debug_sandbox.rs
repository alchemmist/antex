...
use antex_core::windows_sandbox::managed_proxy_routing_for_windows_sandbox;
...
async fn run_command_under_sandbox(
    ...
    if let SandboxType::Windows = sandbox_type {
        ...
        {
            if config.permissions.windows_sandbox_type != antex_sandboxing::SandboxType::WindowsMxc
            {
                let workspace_roots = config
                    .effective_workspace_roots()
                    .iter()
                    .map(antex_utils_path_uri::PathUri::to_abs_path)
    ...
    let mut child = match sandbox_type {
        ...
        SandboxType::Windows => {
            ...
            let antex_self_exe = std::env::current_exe()?;
            let request = SandboxManager::new().transform(SandboxTransformRequest {
                ...
                sandbox: antex_sandboxing::SandboxType::WindowsMxc,
                enforce_managed_network,
                environment_id: None,
                network: network.as_ref(),
                sandbox_policy_cwd: &sandbox_policy_cwd,
                sandbox_exe: Some(antex_self_exe.as_path()),
