...
impl StateRuntime {
    ...
    async fn init_inner(
        ...
        if has_memories_v2
            && let Err(err) = runtime
                .memories_for_version(antex_protocol::MemoryVersion::V2)
                .await
        {
            runtime.close().await;
            return Err(err);
    ...
    pub async fn clear_memory_data_in_sqlite_home(sqlite: &SqliteConfig) -> anyhow::Result<bool> {
        ...
        for version in [
            antex_protocol::MemoryVersion::V1,
            antex_protocol::MemoryVersion::V2,
        ] {
            let path = match version {
                antex_protocol::MemoryVersion::V1 => sqlite.memories_db_path(),
                antex_protocol::MemoryVersion::V2 => sqlite.memories_v2_db_path(),
            };
            ...
            let pool = match version {
                ...
                antex_protocol::MemoryVersion::V2 => sqlite.open_memories_v2_db().await?,
            };
