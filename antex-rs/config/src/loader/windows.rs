use super::windows_antex_system_dir;
use std::path::Path;
...
pub fn probe_windows_system_config_namespace() -> WindowsSystemConfigNamespaceProbe {
    let antex_dir = windows_antex_system_dir();
    let has_system_file = match (
        antex_dir.join("config.toml").try_exists(),
        antex_dir.join("requirements.toml").try_exists(),
    ) {
    ...
    let Some(openai_dir) = antex_dir.parent() else {
        return WindowsSystemConfigNamespaceProbe::CheckError;
    };
    let result = probe_directory(
        openai_dir,
        antex_windows_sandbox::path_has_standard_user_mutation_allow,
    );
    ...
    let result = probe_directory(
        antex_dir.as_path(),
        antex_windows_sandbox::path_or_child_file_has_standard_user_mutation_allow,
    );
    if !matches!(result, WindowsSystemConfigNamespaceProbe::Expected) {
        return result;
    }
...
fn probe_directory(
    ...
    match antex_windows_sandbox::path_has_trusted_system_owner(path) {
        Ok(true) => {}
