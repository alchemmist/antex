...
- Hosts the request/response models and request builders for Responses and related Antex APIs.
...
All HTTP details (URLs, headers, retry/backoff policies, SSE framing) are encapsulated in `antex-api` and `antex-client`. Callers construct prompts/inputs using protocol types and work with typed streams of `ResponseEvent` or other endpoint-specific response values.
