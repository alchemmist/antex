...
impl AnalyticsReducer {
    ...
    fn ingest_app_used(&mut self, input: AppUsedInput, out: &mut Vec<TrackEventRequest>) {
        ...
        let event_params = CodexAppUsedMetadata {
            app: antex_app_metadata(&tracking, app),
            elicitation_type,
        };
    ...
    fn emit_thread_initialized(
        ...
        let is_worktree =
            antex_git_utils::repository_identity(thread.cwd.as_path()).and_then(|_| {
                antex_git_utils::get_git_repo_root(thread.cwd.canonicalize().ok()?.as_path())
