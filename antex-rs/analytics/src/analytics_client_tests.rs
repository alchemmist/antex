...
async fn thread_initialized_classifies_validated_linked_worktrees() {
    ...
    for (cwd, expected) in [
        ...
        start.thread.cwd = antex_utils_absolute_path::AbsolutePathBuf::from_absolute_path(cwd)
                    ...
                    ItemStartedNotification {
                        ...
                        item: {
                            ...
                            if let ThreadItem::CommandExecution { model_context, .. } = &mut item {
                                *model_context =
                                    Some(antex_protocol::items::ModelInvocationContext {
