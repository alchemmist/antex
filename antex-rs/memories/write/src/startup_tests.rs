...
async fn run_memory_phase_one_model_request_test(
    ...
    .await?;
    if version == antex_protocol::MemoryVersion::V2 {
...
async fn run_memory_phase_two_model_request_test(
    ...
    if version == antex_protocol::MemoryVersion::V2 {
        ...
        let phase1_claim = store
            ...
            .await?;
        let antex_state::Stage1JobClaimOutcome::Claimed { ownership_token } = phase1_claim else {
    ...
    match version {
        antex_protocol::MemoryVersion::V1 => seed_required_memory_artifacts(&root).await?,
        antex_protocol::MemoryVersion::V2 => {
...
-> anyhow::Result<()> {
    ...
    let home = Arc::new(TempDir::new()?);
    let mut memories = startup_test_memories_config();
    memories.version = antex_protocol::MemoryVersion::V2;
    let request = run_memory_phase_one_model_request_test(&server, home, memories).await?;
