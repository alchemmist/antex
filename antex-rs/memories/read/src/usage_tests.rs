...
use antex_protocol::MemoryVersion;
...
fn attributes_mixed_version_reads_to_their_roots() {
    let command = "cat /tmp/.antex/memories/MEMORY.md \
        && sed -n '1,20p' /tmp/.antex/memories_v2/rollout_summaries/thread.md \
        && cat /tmp/.antex/memories/raw_memories.md \
        && cat /tmp/.antex/memories_v2/skills/testing/SKILL.md";
    assert_eq!(
        memories_usage_from_command(command),
        vec![
