...
async fn explicit_update_migrates_running_and_stopped_installations() {
    for (running, local) in [(false, false), (true, false), (false, true), (true, true)] {
        ...
        if local {
            ...
            std::fs::copy(&legacy.managed_antex_bin, package.join("antex")).unwrap();
        ...
        b"# ANTEX_INSTALL_IF_LATEST\ntest \"$ANTEX_INSTALL_DEFER_SELECTION\" = 0 && test \"$ANTEX_INSTALL_DAEMON_ONLY\" = 0\n".to_vec(),
    ));
        super::update_once(
            &scheduled,
            &legacy,
            &executable_identity(&legacy.managed_antex_bin)
                .await
                .unwrap(),
            &mut test_terminate(),
        ...
        let legacy_bin = std::fs::read(&legacy.managed_antex_bin).unwrap();
        ...
        let script = format!(
            r#"# ANTEX_INSTALL_DEFER_SELECTION
set -eu
test "$ANTEX_INSTALL_DEFER_SELECTION" = 1
test "$ANTEX_INSTALL_IF_CURRENT" = 0
test "$ANTEX_INSTALL_DAEMON_ONLY" = 1
test "$ANTEX_INSTALL_IF_LATEST" = 0
root="$ANTEX_HOME/packages/app-server-daemon"
mkdir -p "$root/releases/{release}/bin"
printf '#!/bin/sh\nif [ "$1" = --version ]; then echo antex 1.0.0; else exit 2; fi\n' > "$root/releases/{release}/bin/antex"
chmod +x "$root/releases/{release}/bin/antex"
...
"#
        ...
        managed_antex_path: dedicated.join("current/bin/antex"),
        ...
        assert_eq!(
            std::fs::read(&legacy.managed_antex_bin).unwrap(),
            legacy_bin
        );
        assert_eq!(
        ...
        let selected = Daemon {
            ...
            managed_antex_bin: dedicated.join("current/bin/antex"),
            ..legacy.clone()
        };
...
fn manual_update_daemon(home: &TempDir) -> (Daemon, String) {
    ...
    let bin = standalone.join("releases").join(&release).join("antex");
    std::fs::create_dir_all(bin.parent().expect("binary parent")).expect("release directory");
    std::fs::write(
        &bin,
        b"#!/bin/sh\nif [ \"$1\" = '--version' ]; then echo antex 1.0.0; else exec sleep 30; fi\n",
    )
    ...
    (
        Daemon {
            ...
            managed_antex_bin: standalone.join("current/antex"),
        },
        release,
    )
}
...
async fn manual_request_retries_after_updater_replacement() {
    ...
    antex_uds::prepare_private_socket_directory(socket_path.parent().expect("socket parent"))
        .await
        .expect("socket directory");
    let mut listener = antex_uds::UnixListener::bind(&socket_path)
        .await
        .expect("old updater socket");
    let expected = UpdateOutput {
        status: UpdateStatus::NoUpdate,
        managed_antex_path: daemon.managed_antex_bin.clone(),
    ...
    let server = tokio::spawn(async move {
        ...
        let mut successor = antex_uds::UnixListener::bind(&socket_path)
            .await
...
async fn manual_request_recovers_when_one_shot_updater_exits() {
    ...
    std::fs::remove_file(&daemon.managed_antex_bin).expect("remove selected binary");
    let result = tokio::time::timeout(
...
async fn unsupported_request_preserves_updater_schedule() {
    ...
    let identity = executable_identity(&daemon.managed_antex_bin)
        .await
    ...
    let mut pinned = antex_uds::UnixStream::connect(&socket_path).await.unwrap();
    ...
    std::fs::remove_file(&daemon.managed_antex_bin).expect("remove selected binary");
    let mut malformed = antex_uds::UnixStream::connect(&socket_path)
        .await
...
async fn test_control_server(
    ...
    std::fs::create_dir_all(daemon.socket_path.parent().expect("socket parent"))
        .expect("socket directory");
    let mut listener = antex_uds::UnixListener::bind(&daemon.socket_path)
        .await
...
async fn check_manual_update_restart(package_directory: &str) {
    ...
    if local_package {
        ...
        daemon.managed_antex_bin = standalone.join("current/antex");
        release = local;
    }
    let daemon = std::sync::Arc::new(daemon);
    ...
    let guard = if local_package {
        "ANTEX_INSTALL_IF_CURRENT"
    } else {
        "ANTEX_INSTALL_IF_LATEST"
    };
    let install_binary = if local_package {
        // Same binary and version, but a different package: it must still restart.
        format!(
            "cp '{root}/releases/{release}/antex' '{root}/releases/{next}/bin/antex'",
            root = standalone.display()
        )
    } else {
        format!(
            r#"printf '#!/bin/sh\nif [ "$1" = --version ]; then echo antex 1.1.0; else exec sleep 30; fi\n' > '{root}/releases/{next}/bin/antex'"#,
            root = standalone.display()
        )
    };
    ...
    let script = format!(
        "#!/bin/sh\n# ANTEX_INSTALL_IF_LATEST ANTEX_INSTALL_IF_CURRENT ANTEX_INSTALL_DAEMON_ONLY\nif [ \"$ANTEX_UPDATE_FROM_RELEASE\" = '{next}' ]; then exit 0; fi\ntest \"${guard}\" = 1 || exit 4\ntest \"$ANTEX_UPDATE_FROM_RELEASE\" = '{release}' || exit 5\ntouch '{ready}'\nwhile [ ! -e '{proceed}' ]; do sleep .05; done\nmkdir -p '{root}/releases/{next}/bin'\n{install_binary}\nchmod +x '{root}/releases/{next}/bin/antex'\nln -sfn 'releases/{next}' '{root}/current'\nprintf '{next}' > '{root}/auto-update-version'\n",
        root = standalone.display(),
        ready = ready.display(),
    ...
    let mut queued = antex_uds::UnixStream::connect(&daemon.manual_update_socket_path())
        .await
    ...
    assert_eq!(
        ...
        standalone.join("current/bin/antex")
    );
    ...
    let no_op = FakeInstallerHttp::new(InstallerResponse::Success(
        b"#!/bin/sh\n# ANTEX_INSTALL_IF_LATEST ANTEX_INSTALL_DAEMON_ONLY\nexit 0\n".to_vec(),
    ));
    use std::io::Write;
...
function Test-Installer {
    ...
    if ($env:ANTEX_INSTALL_DAEMON_ONLY -ne '1') { throw 'wrong package destination' }
    if ($env:ANTEX_INSTALL_IF_CURRENT -ne '1' -or $env:ANTEX_INSTALL_IF_LATEST -ne '0') { throw 'wrong update guard' }
}
Test-Installer
"#
        .to_vec(),
...
async fn daemon_owned_updates_require_and_request_an_isolated_installer() {
    ...
    daemon.managed_antex_bin = root.join("current/antex");
    let identity = executable_identity(&daemon.managed_antex_bin)
        .await
        .unwrap();
    ...
    let current = FakeInstallerHttp::new(InstallerResponse::Success(
        b"# ANTEX_INSTALL_IF_LATEST\ntest \"$ANTEX_INSTALL_IF_LATEST\" = 1 && test \"$ANTEX_INSTALL_DAEMON_ONLY\" = 1\n".to_vec(),
    ));
    let output = manual_update_once(
        &current,
        &daemon,
    ...
    let no_op = FakeInstallerHttp::new(InstallerResponse::Success(
        b"# ANTEX_INSTALL_IF_CURRENT ANTEX_INSTALL_DAEMON_ONLY\nexit 0\n".to_vec(),
    ));
    let script = format!(
        r#"# ANTEX_INSTALL_IF_CURRENT ANTEX_INSTALL_DAEMON_ONLY
set -eu
test "$ANTEX_INSTALL_IF_CURRENT" = 1
test "$ANTEX_INSTALL_IF_LATEST" = 0
test "$ANTEX_INSTALL_DAEMON_ONLY" = 1
test "$ANTEX_RELEASE" = latest
test "$ANTEX_UPDATE_FROM_RELEASE" = '{release}'
