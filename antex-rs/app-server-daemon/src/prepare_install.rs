...
use antex_install_context::AntexPackageManifest;
...
pub async fn update_from_cli(
    ...
    let managed_antex_path = daemon.current_managed_codex_bin()?;
    Ok(Some(crate::UpdateOutput {
        status: crate::UpdateStatus::Updated,
        installed_version: Some(managed_install::managed_antex_version(&managed_antex_path).await?),
        running_version: crate::client::probe(&daemon.socket_path)
            .await
            .ok()
            .map(|info| info.app_server_version),
        managed_antex_path,
        message: "The CLI package is selected and pinned. Run `antex app-server daemon update` to return to production updates.".to_string(),
    }))
}
...
async fn prepare_from_package(
    ...
    anyhow::ensure!(
        daemon.managed_antex_bin.starts_with(&previous_root),
    ...
    if mode == InstallMode::Missing {
        ...
        {
            daemon.ensure_managed_antex_bin()?;
            return Ok(true);
        }
    } else {
        anyhow::ensure!(
            previous_root.join("current").symlink_metadata().is_ok(),
            "no daemon package is selected; run `antex app-server daemon start` first"
        );
    }
    ...
    let selected = managed_install::managed_antex_bin(home);
    ...
    anyhow::ensure!(
        ...
        "ANTEX_HOME must be outside the source CLI package"
    );
    let manifest_bytes = std::fs::read(source.join("antex-package.json"))?;
    let manifest: AntexPackageManifest = serde_json::from_slice(&manifest_bytes)?;
    ...
    if mode == InstallMode::Replace {
        if !confirm(&InstallRequest {
            ...
            installed_version: tokio::time::timeout(
                ...
                managed_install::managed_antex_version(&selected),
            )
            .await
            .ok()
            .and_then(Result::ok),
    ...
    if mode == InstallMode::Missing && managed_install::managed_antex_bin(home).is_file() {
        return Ok(true);
    }
    anyhow::ensure!(
    ...
    let staged_exe = if cfg!(target_os = "macos") && stage.path().join("AntexCLI.app").is_dir() {
        stage.path().join("AntexCLI.app/Contents/MacOS/antex")
    } else {
        stage.path().join(entrypoint)
    };
    anyhow::ensure!(
        package_tree(source, /*destination*/ None)? == digest
            && std::fs::read(stage.path().join("antex-package.json"))? == manifest_bytes
    ...
    if release.try_exists()? {
        ...
        #[cfg(unix)]
        if !stage.path().join("antex").exists() {
            std::os::unix::fs::symlink("bin/antex", stage.path().join("antex"))?;
        }
    ...
    if backend.is_some() {
        let selected = Daemon {
            ...
            managed_antex_bin: root.join("current").join(entrypoint),
            ..daemon.clone()
        };
        selected.start_managed_backend(settings).await.context(
            "daemon package selected but could not start; retry with `antex app-server daemon start`",
        )?;
...
fn package_tree(root: &Path, destination: Option<&Path>) -> Result<String> {
    ...
    while let Some(path) = paths.pop() {
        ...
        if cfg!(unix)
            && relative == Path::new("antex")
            && std::fs::read_link(&path).ok().as_deref() == Some(Path::new("bin/antex"))
        {
            continue;
        }
        anyhow::ensure!(
...
fn validate_package(root: &Path) -> Result<()> {
    ...
    if cfg!(windows) {
        ...
        names.push("antex-resources/bwrap");
    }
    for name in names {
