...
async fn run_with_http(
    ...
    let mut listener = Some(antex_uds::UnixListener::bind(&socket_path).await?);
    #[cfg(windows)]
    updater.mark_ready().await?;
    let needs_managed_handoff =
        match resolved_managed_antex_bin(&daemon.current_managed_codex_bin()?).await {
            Ok(managed_bin) => {
...
async fn adopt_managed_updater(
    ...
    listener: &mut Option<antex_uds::UnixListener>,
) -> Result<UpdateLoopControl> {
    let managed_bin = resolved_managed_antex_bin(&daemon.current_managed_codex_bin()?).await?;
    ...
    #[cfg(windows)]
    {
        ...
        if let Err(err) = replacement.replace_current_updater().await {
            ...
            *listener = Some(antex_uds::UnixListener::bind(&socket_path).await?);
            return Err(err);
        }
...
async fn update_once(
    ...
    let codex_home = package_root
        ...
        .context("daemon package root has no Antex home")?;
    ...
    if package_root.ends_with("app-server-daemon") {
        anyhow::ensure!(
            script
                .windows(b"ANTEX_INSTALL_DAEMON_ONLY".len())
                .any(|window| window == b"ANTEX_INSTALL_DAEMON_ONLY"),
    ...
    let managed_antex_bin =
        resolved_managed_antex_bin(&daemon.current_managed_codex_bin()?).await?;
    let restart_mode = match trigger {
        ...
        UpdateTrigger::Scheduled
            if executable_identity(&managed_antex_bin).await? != *running_updater_identity =>
        {
            RestartMode::Always
        }
    ...
    loop {
        ...
        match daemon
            .try_restart_if_running(restart_mode, &managed_antex_bin)
            .await?
        {
...
fn selected_release(daemon: &Daemon) -> Result<(std::path::PathBuf, std::path::PathBuf, String)> {
    let home = daemon
        ...
        .parent()
        .and_then(Path::parent)
        .context("daemon settings path has no Antex home")?;
...
async fn run_installer_script(
    ...
    let mut child = command
        .env(
            ...
            package_root
                .parent()
                .and_then(Path::parent)
                .context("package root has no Antex home")?,
        )
        .env("ANTEX_INSTALL_DEFER_SELECTION", defer_selection)
        .env(
            "ANTEX_INSTALL_DAEMON_ONLY",
            if package_root.ends_with("app-server-daemon") {
                "1"
            } else {
                "0"
        ...
        .env("ANTEX_RELEASE", "latest")
        .env("ANTEX_NON_INTERACTIVE", "1")
        .env("ANTEX_INSTALL_IF_LATEST", latest_guard)
        .env("ANTEX_INSTALL_IF_CURRENT", current_guard)
        .env("ANTEX_UPDATE_FROM_RELEASE", previous_release)
        .kill_on_drop(true)
        .stdin(Stdio::piped())
        .stdout(Stdio::null())
        .stderr(Stdio::null())
        .spawn()
        .context("failed to invoke standalone Antex updater")?;
    #[cfg(windows)]
    let _installer_job = crate::backend::windows::installer_job(&child)?;
    let mut stdin = child
        .stdin
        .take()
        .context("standalone Antex updater stdin was unavailable")?;
    #[cfg(unix)]
    ...
    write_result
        ...
        .context("failed to pass standalone Antex updater to shell")?;
    #[cfg(unix)]
    ...
    let status = status.context("failed to wait for standalone Antex updater")?;

    if status.success() {
        Ok(UpdateLoopControl::Continue)
    } else {
        anyhow::bail!("standalone Antex updater exited with status {status}")
    }
}
...
async fn fetch_installer_script(http: &impl InstallerHttp) -> Result<Vec<u8>> {
    match http.get(INSTALL_URL).await? {
        ...
        InstallerResponse::Unsuccessful { status } => {
            anyhow::bail!("standalone Antex updater request failed with status {status}")
        }
    }
}
...
impl InstallerHttp for RouteAwareClientPool {
    async fn get(&self, url: &str) -> Result<InstallerResponse> {
        let response = RouteAwareClientPool::get(self, url)
            .send()
            .await
            .context("failed to fetch standalone Antex updater")?;
        ...
        let body = response
            .bytes()
            .await
            .context("failed to read standalone Antex updater")?
            .to_vec();
