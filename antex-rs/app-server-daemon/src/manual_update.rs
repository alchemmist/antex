...
use crate::managed_install::managed_antex_version;
...
pub(super) async fn unsupported(daemon: &Daemon) -> Result<UpdateOutput> {
    ...
    Ok(UpdateOutput {
        ...
        installed_version: managed_antex_version(&managed_antex_path).await.ok(),
...
pub(super) async fn run(
    ...
    let previous_identity = executable_identity(&managed_antex_path).await?;
    let (control, restart) =
    ...
    let installed_version = Some(managed_antex_version(&current_managed_codex_path).await?);
    ...
    Ok(UpdateOutput {
        ...
        managed_antex_path: current_managed_codex_path,
        installed_version,
        running_version,
