...
impl Daemon {
    fn from_environment() -> Result<Self> {
        ...
        let managed_antex_bin = managed_antex_bin(codex_home.as_path());
        // Old CLIs must not mistake a daemon-owned installation for their backend.
        let (pid_file, update_pid_file) =
            if managed_antex_bin.starts_with(codex_home.as_path().join("packages/standalone")) {
    ...
    fn recovery_file(&self) -> Result<PathBuf> {
        Ok(antex_app_server_transport::daemon_recovery_file_path(
            self.settings_file
                .parent()
    ...
    fn current_installation(&self) -> Result<Self> {
        let managed_antex_bin = self.current_managed_codex_bin()?;
        let home = self
            .settings_file
            .parent()
            .and_then(Path::parent)
            .context("daemon settings path has no Codex home")?;
        let (pid, updater) = if managed_antex_bin.starts_with(home.join("packages/standalone")) {
            (LEGACY_PID_FILE_NAME, LEGACY_UPDATE_PID_FILE_NAME)
        } else {
            (DAEMON_PID_FILE_NAME, DAEMON_UPDATE_PID_FILE_NAME)
        };
    ...
    async fn start(&self) -> Result<LifecycleOutput> {
        ...
        let (status, backend, pid, info) = if let Ok(info) = client::probe(&self.socket_path).await
        {
            ...
            managed.managed_antex_bin = self.current_managed_codex_bin()?;
            managed.ensure_managed_antex_bin()?;
    ...
    pub(crate) async fn try_restart_if_running(
        ...
        let outcome = if let Some(backend) = self.running_backend_instance(&settings).await? {
            ...
            if !self.is_stable_standalone_release()?
                || managed_install::resolved_managed_antex_bin(&self.current_managed_codex_bin()?)
                    .await
                    .ok()
                    .as_deref()
                    != Some(managed_antex_bin)
            {
            ...
            let mode = if mode == RestartMode::IfBinaryOrVersionChanged {
                let managed_identity =
                    managed_install::executable_identity(managed_antex_bin).await?;
    ...
    async fn bootstrap_locked(&self, options: BootstrapOptions) -> Result<BootstrapOutput> {
        ...
        let managed_antex_version = managed.managed_antex_version_best_effort().await;
        Ok(BootstrapOutput {
            status: BootstrapStatus::Bootstrapped,
            backend: BackendKind::Pid,
            auto_update_enabled,
            remote_control_enabled: settings.remote_control_enabled,
            managed_antex_path: managed.managed_antex_bin,
    ...
    async fn ensure_managed_updater(&self, settings: &DaemonSettings) -> Result<bool> {
        ...
        let Ok(antex_bin) =
            managed_install::resolved_managed_antex_bin(&self.managed_antex_bin).await
        else {
            if !self.has_latest_selection_marker() {
                updater.stop().await?;
            }
        ...
        if !managed_install::supports_daemon_update_loop(&antex_bin).await
            || !self.is_stable_standalone_release()?
            || !managed_install::resolved_managed_antex_bin(&self.managed_antex_bin)
                .await
                .is_ok_and(|selected| selected == antex_bin)
        {
            if !self.has_latest_selection_marker() {
                updater.stop().await?;
            }
        ...
        backend::pid_update_loop_backend(self.backend_paths_with_bin(settings, &antex_bin))
            .start()
            .await?;
        Ok(true)
    }
    ...
    fn current_managed_codex_bin(&self) -> Result<PathBuf> {
        ...
        let home = self
            ...
            .and_then(Path::parent)
            .context("daemon settings path has no Codex home")?;
        Ok(managed_install::managed_antex_bin(home))
    }
    ...
    async fn is_bootstrapped(&self, settings: &DaemonSettings) -> Result<bool> {
        if !settings.auto_update_enabled
            || !self.is_stable_standalone_release()?
            || !managed_install::supports_daemon_update_loop(&self.managed_antex_bin).await
        {
    ...
    fn ensure_managed_antex_bin(&self) -> Result<()> {
        ...
        Err(anyhow!(
            "daemon executable not found at {managed_antex_path}; repair the existing installation, or run `antex app-server daemon start` to install a missing daemon"
        ))
    }
...
mod tests {
    ...
    async fn waiting_lifecycle_command_uses_migrated_installation() {
        ...
        let daemon = Daemon {
            ...
            managed_antex_bin: super::managed_antex_bin(home.path()),
        };
        ...
        assert_eq!(
            (output.status, output.managed_antex_path),
            (
                LifecycleStatus::NotRunning,
                super::managed_antex_bin(home.path())
            )
        );
    }
    ...
    async fn stop_and_fresh_start_discard_pending_thread_restore() {
        let home = TempDir::new().expect("home");
        let state = home.path().join("app-server-daemon");
        antex_uds::prepare_private_socket_directory(&state)
            .await
        ...
        let daemon = Daemon {
            ...
            operation_lock_file: state.join("daemon.lock"),
            settings_file: state.join("settings.json"),
            managed_antex_bin: home.path().join("antex"),
        };
        antex_app_server_transport::daemon_recovery::write_candidates(
    ...
    async fn managed_local_backend_counts_as_bootstrapped_without_updater() {
        ...
        let local_bin = standalone.join("local-main/bin/antex");
        ...
        let daemon = Daemon {
            ...
            operation_lock_file: state.join("daemon.lock"),
            settings_file: state.join("settings.json"),
            managed_antex_bin: standalone.join("current/bin/antex"),
        };
