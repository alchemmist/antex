...
fn package(root: &Path, version: &str) -> PathBuf {
    ...
    std::fs::write(
        root.join("antex-package.json"),
        serde_json::json!({
            "version": version, "target": target, "entrypoint": "bin/antex"
        })
        .to_string(),
    )
    .expect("manifest");
...
async fn seeds_full_package() {
    ...
    assert_eq!(
        std::fs::read(selected.join("antex-resources/nested/runtime")).expect("runtime"),
        b"runtime"
    );
    assert_eq!(
...
async fn incomplete_source_fails_without_selecting_it() {
    ...
    std::fs::remove_file(source.join("bin/antex-code-mode-host")).expect("remove helper");
    let home = temp.path().join("home");
    ...
    assert!(error.to_string().contains("bin/antex-code-mode-host"));
...
async fn provisioned_macos_bundle_seeds_from_its_running_executable() {
    ...
    let bundle = source.join("AntexCLI.app/Contents/MacOS/antex");
    ...
    assert_eq!(
        std::fs::read(selected.join("AntexCLI.app/Contents/MacOS/antex"))
...
async fn legacy_selection_is_not_migrated_or_refreshed() {
    ...
    .unwrap();
    assert_eq!(
        crate::managed_install::managed_antex_bin(&home)
            .canonicalize()
            .unwrap(),
...
async fn explicit_selection_requires_unchanged_cli_and_pins_all_versions() {
    for legacy in [false, true] {
        ...
        for version in [
            ...
            std::fs::write(
                source.join("antex-resources/nested/runtime"),
                previous.to_string_lossy().as_bytes(),
            )
            .unwrap();
            let before = std::fs::read(previous.join("bin/antex")).unwrap();
            assert!(
                !prepare_from_package(
                    &daemon,
                    &settings,
            ...
            assert_eq!(
                std::fs::read(selected.join("bin/antex")).unwrap(),
                std::fs::read(&bin).unwrap()
            );
            assert_eq!(std::fs::read(previous.join("bin/antex")).unwrap(), before);
