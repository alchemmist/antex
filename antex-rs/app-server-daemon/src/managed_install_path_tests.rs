...
fn discovers_package_and_legacy_installs() {
    ...
    assert_eq!(
        ...
        home.path()
            ...
            .join(super::managed_antex_file_name())
    );
...
fn updater_only_runs_for_stable_installer_owned_releases() {
    ...
    let managed = release.join("bin/antex");
    ...
    let alpha_managed = alpha.join("bin/antex");
    ...
    let local_managed = local.join("bin/antex");
...
async fn older_managed_binary_does_not_claim_updater_support() {
    ...
    let binary = temp.path().join("antex");
