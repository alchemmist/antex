...
pub(super) async fn run(http: &impl InstallerHttp, legacy: &Daemon) -> Result<UpdateOutput> {
    ...
    anyhow::ensure!(
        script
            .windows(b"ANTEX_INSTALL_DEFER_SELECTION".len())
            .any(|window| window == b"ANTEX_INSTALL_DEFER_SELECTION"),
    ...
    let version = managed_install::managed_antex_version(&binary).await?;
    anyhow::ensure!(
    ...
    let running_version = if running.is_some() {
        selected.start_managed_backend(&settings).await.context(
            "daemon migrated but could not start; retry with `antex app-server daemon start`",
        )?;
        Some(
            selected
                .wait_until_ready()
                .await
                .context(
                    "daemon migrated but is not ready; retry with `antex app-server daemon start`",
                )?
                .app_server_version,
        )
    } else {
    ...
    Ok(UpdateOutput {
        ...
        running_version,
        managed_antex_path: selected.managed_antex_bin,
