...
pub(crate) fn is_stable_standalone_release(codex_home: &Path, antex_bin: &Path) -> bool {
    ...
    components.len() == 3
        ...
        && std::fs::canonicalize(antex_bin).is_ok_and(|bin| bin.starts_with(&release))
}

/// Older managed binaries can serve app-server requests without owning an updater.
pub(crate) async fn supports_daemon_update_loop(antex_bin: &Path) -> bool {
    supports_daemon_command(antex_bin, &["pid-update-loop", "--help"]).await
}

/// Probe an internal daemon command without running a long-lived process.
pub(crate) async fn supports_daemon_command(antex_bin: &Path, args: &[&str]) -> bool {
