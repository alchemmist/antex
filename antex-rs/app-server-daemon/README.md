...
default. Edit `ANTEX_HOME/app-server-daemon/settings.json` to change this:
...
it copies the invoking package into `ANTEX_HOME/packages/app-server-daemon` and
...
It records the daemon settings under `ANTEX_HOME/app-server-daemon/`, starts app-server as a
...
New daemons use `ANTEX_HOME/packages/app-server-daemon/current/bin/codex`
...
Previously launched legacy daemons retain `ANTEX_HOME/packages/standalone/current`,
