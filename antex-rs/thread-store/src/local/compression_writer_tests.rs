...
use antex_rollout::WriterLockCoordinator;
...
async fn compress(home: &Path) -> TestResult<()> {
    ...
    antex_rollout::spawn_rollout_compression_worker(home.to_path_buf());
    // The marker proves startup; the maintenance lock proves every blocking job finished.
    wait_until(|| {
        marker.exists()
            && antex_rollout::try_acquire_rollout_maintenance_lock(home)
                .unwrap()
                .is_some()
    })
    .await;
...
async fn metadata_updates_share_ownership_and_resume_compressed_rollouts() -> TestResult<()> {
    ...
    let mut expected = antex_rollout::read_session_meta_line(&path).await?;
