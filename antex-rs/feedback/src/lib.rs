...
impl FeedbackAttachmentPath {
    ...
    pub fn read_attachment(&self, max_bytes: usize) -> io::Result<Option<FeedbackAttachment>> {
        let rollout_path = antex_rollout::rollout_id_from_path(&self.path)
            .map(|_| antex_rollout::plain_rollout_path(&self.path));
        let buffer = if rollout_path.is_some() {
            let Some(buffer) =
                antex_rollout::read_rollout_prefix(&self.path, max_bytes.saturating_add(1))?
            else {
                return Ok(None);
            };
            buffer
