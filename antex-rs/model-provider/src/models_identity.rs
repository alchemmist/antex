...
pub(crate) fn identity(
    ...
    let mut provider = provider_info.to_api_provider(auth.map(AntexAuth::auth_mode))?;
    ...
    field(format!("{:?}", auth.map(AntexAuth::api_auth_mode)).as_bytes());
    ...
    let has_stable_account = auth.is_some_and(|auth| {
        matches!(
            auth,
            AntexAuth::Chatgpt(_) | AntexAuth::ChatgptAuthTokens(_) | AntexAuth::AgentIdentity(_)
