...
fn chatgpt_auth(email: &str, user: &str, account: &str, plan: &str, signature: &str) -> AntexAuth {
...
fn cache_identity_tracks_account_email_user_plan_and_auth_mode_but_not_token_refresh() {
    ...
    provider.http_headers = Some(std::collections::HashMap::from([(
        antex_login::default_client::RESIDENCY_HEADER_NAME.to_string(),
        "us".into(),
    )]));
    ...
    for auth in [
        ...
        AntexAuth::from_api_key("api-key"),
    ] {
...
fn cache_identity_tracks_provider_routing_and_effective_api_credentials() {
    let auth = AntexAuth::from_api_key("first-key");
    ...
    let other_key = AntexAuth::from_api_key("second-key");
