...
use antex_app_server_transport::daemon_recovery;
use antex_app_server_transport::daemon_recovery_file_path;
...
async fn managed_shutdown_skips_nonpersistent_threads_and_tolerates_save_failure(
    ...
    let source = if matches!(scenario, SnapshotScenario::Child) {
        antex_protocol::protocol::SessionSource::SubAgent(
            antex_protocol::protocol::SubAgentSource::ThreadSpawn {
                parent_thread_id: antex_protocol::ThreadId::new(),
                depth: 1,
                agent_path: None,
                agent_nickname: None,
                agent_role: None,
    ...
    let stored = if matches!(scenario, SnapshotScenario::Parented) {
        app_test_support::create_fake_parented_rollout_with_source(
            ...
            /*git_info*/ None,
            source,
            antex_protocol::SessionId::new(),
            antex_protocol::ThreadId::new(),
        )?
    } else {
...
fn spawn_server(home: &Path, socket_path: &Path) -> Result<Child> {
    let binary = antex_utils_cargo_bin::cargo_bin("antex-app-server")?;
    Ok(Command::new(binary)
        ...
        .arg("--managed-daemon")
        .env(
            antex_app_server_transport::DAEMON_SHUTDOWN_SOCKET_ENV,
            socket_path,
        )
        .stdin(Stdio::null())
        .stdout(Stdio::null())
