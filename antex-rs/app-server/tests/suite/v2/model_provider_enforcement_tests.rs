...
async fn provider_requirement_changes_reject_inputs_to_existing_threads(
    ...
    MockResponsesConfig::new(gateway.uri())
        .enable_feature(antex_features::Feature::Goals)
        .write(home.path())?;
...
"#,
    ...
    let id = server
        ...
        .await?;
    let _: antex_app_server_protocol::ThreadGoalSetResponse = server.read_response(id).await?;
    ...
    let id = server
        ...
        .await?;
    let goal: antex_app_server_protocol::ThreadGoalGetResponse = server.read_response(id).await?;
    ...
    let id = server
        ...
        .await?;
    let _: antex_app_server_protocol::ThreadGoalClearResponse = server.read_response(id).await?;
    let id = server
        .send_raw_request(
            "thread/queue/start",
            Some(json!({
    ...
    let id = server
        ...
        .await?;
    let _: antex_app_server_protocol::ThreadQueueDeleteResponse = server.read_response(id).await?;
