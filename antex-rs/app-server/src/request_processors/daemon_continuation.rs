...
use antex_app_server_transport::daemon_recovery::InterruptedTurn;
...
impl ThreadRequestProcessor {
    pub(crate) async fn continue_daemon_turn(&self, thread_id: &str, saved: InterruptedTurn) {
        ...
        let history = match antex_rollout::RolloutRecorder::load_rollout_items(&path).await {
        ...
        if environment.environment_id != antex_exec_server::LOCAL_ENVIRONMENT_ID
