...
impl WindowsSandboxRequestProcessor {
    ...
    pub(crate) async fn windows_sandbox_readiness(
        ...
        #[cfg(target_os = "windows")]
        if antex_windows_sandbox::registered_core_requested()
            && matches!(
            ...
            tokio::spawn(async move {
                ...
                processor.registration_refresh.get_or_init(|| async {
                    ...
                    let result = tokio::task::spawn_blocking(move || -> anyhow::Result<()> {
                        if !antex_windows_sandbox::registered_core_needs_refresh(&config.codex_home)? {
                            return Ok(());
                        }
                        ...
                        let (settings, listeners) = match config.permissions.network.as_ref() {
                            ...
                            None => (
                                antex_windows_sandbox::WindowsSandboxProvisioningSettings::from_environment(&policy, &env_map),
                                antex_windows_sandbox::WindowsSandboxProxyListeners::from_environment(&policy, &env_map),
                            ),
                        };
                        antex_windows_sandbox::refresh_registered_core_via_service(
        ...
        tokio::spawn(async move {
            ...
            let setup_result = async {
                ...
                if setup_mode == CoreWindowsSandboxSetupMode::Elevated
                    && (antex_windows_sandbox::registered_core_requested()
                    ...
                    && !antex_login::is_workload_identity_selected()
                {
                    ...
                    if let Some((settings, listeners)) = provisioning {
                        ...
                        tokio::task::spawn_blocking(move || -> anyhow::Result<()> {
                            let Ok(permissions) = antex_windows_sandbox::ResolvedWindowsSandboxPermissions::try_from_permission_profile_for_workspace_roots(
                                &service_setup_request.permission_profile,
                                &service_setup_request.workspace_roots,
                            ) else {
                                anyhow::ensure!(!antex_windows_sandbox::registered_core_requested(),
