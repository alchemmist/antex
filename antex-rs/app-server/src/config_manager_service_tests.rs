...
async fn allowed_login_methods_follow_current_forced_workspaces() -> Result<()> {
    ...
    let auth = antex_login::AuthManager::shared_from_config(
...
async fn permission_config_reload_merges_session_layers() -> Result<()> {
    ...
    let service = ConfigManager::new(
        ...
        /*strict_config*/ false,
        CloudConfigBundleLoader::default(),
        antex_arg0::Arg0DispatchPaths {
            main_execve_wrapper_exe: Some(wrapper),
            ..Default::default()
        },
        std::sync::Arc::new(antex_config::NoopThreadConfigLoader),
    );

    let mut config = service
        .load_with_overrides(
            /*request_overrides*/ None,
            antex_core::config::ConfigOverrides {
