...
use antex_app_server_protocol::UserVerificationCancelResponse;
...
impl MessageProcessor {
    ...
    pub(crate) async fn daemon_recovery_snapshot(
        &self,
    ) -> antex_app_server_transport::daemon_recovery::RecoverySnapshot {
        self.thread_processor.daemon_recovery_snapshot().await
    }

    pub(crate) async fn restore_daemon_threads(
        &self,
        mut snapshot: antex_app_server_transport::daemon_recovery::RecoverySnapshot,
    ) {
        for thread_id in snapshot.loaded {
            ...
            if let Err(err) = self
                ...
                .thread_resume(
                    ...
                    antex_app_server_protocol::ThreadResumeParams {
    ...
    async fn dispatch_initialized_client_request(
        ...
        let (turn_admission, recheck_turn_admission) = match &antex_request {
