...
use antex_config::loader::load_managed_requirements_state;
...
impl ConfigManager {
    ...
    pub(crate) async fn load_permission_config_for_thread(
        ...
        for layer in thread_config.config_layer_stack.layers_low_to_high() {
            if matches!(layer.name, antex_config::ConfigLayerSource::SessionFlags) {
                antex_config::merge_toml_values(&mut session_flags, &layer.config);
            }
        }
