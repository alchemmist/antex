...
use antex_protocol::items::ModelInvocationContext;
    ...
    match msg {
        ...
        EventMsg::GuardianAssessment(assessment) => {
            ...
            if assessment.status == antex_protocol::protocol::GuardianAssessmentStatus::InProgress
                && matches!(
                    ...
                    Some(
                        antex_protocol::approvals::GuardianReviewReason::ElevatedRisk
                            | antex_protocol::approvals::GuardianReviewReason::StaleScore
                            | antex_protocol::approvals::GuardianReviewReason::IncompatibleCompaction
                            | antex_protocol::approvals::GuardianReviewReason::ScoringFailure
                            | antex_protocol::approvals::GuardianReviewReason::AuthorizationChanged
                    )
                )
            {
                outgoing
        ...
        EventMsg::ElicitationRequest(request) => {
            let user_verification = matches!(
                &request.request,
                antex_protocol::approvals::ElicitationRequest::UserVerification { .. }
            );
            let permission_guard = thread_watch_manager
                .note_permission_requested(&conversation_id.to_string())
                .await;
