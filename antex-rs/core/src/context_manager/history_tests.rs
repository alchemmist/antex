...
fn conversation_history_snapshot_binds_review_mode_and_hash_to_the_latest_item(
    ...
    unknown.metadata = latest_hash.map(|hash| AntexHarnessMetadata {
...
fn passthrough_metadata_does_not_change_context_estimates() {
    for output in [
        ...
        for turn_metadata in [
            ...
            let mut call = antex_protocol::models::ExecutedToolCall::new(
                "test_tool".to_string(),
                serde_json::json!({"input": "x".repeat(8 * 1024)}),
            );
            call.set_tool_result_metadata(antex_protocol::models::ToolResultMetadata::new(
