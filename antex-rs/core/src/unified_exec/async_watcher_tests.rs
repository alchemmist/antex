...
async fn exit_watcher_waits_for_late_network_denial_before_classifying_end() -> anyhow::Result<()> {
    ...
    model_info.truncation_policy = antex_protocol::openai_models::TruncationPolicyConfig {
    ...
    assert_eq!(
        ...
        Some(antex_utils_output_truncation::formatted_truncate_text(
            "LATE_DENIAL",
            antex_utils_output_truncation::TruncationPolicy::Bytes(4),
        ))
    );
    assert!(
