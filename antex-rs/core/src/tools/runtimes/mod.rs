...
use antex_shell_command::shell_snapshot::posix_env_path_expansion_function;
...
pub(crate) fn prepare_brokered_shell_snapshot_env(
            ...
            .is_some_and(|active| active == "1")
    {
        if !cfg!(windows) {
            ...
            for key in antex_network_proxy::brokered_credential_value_env_keys(env) {
            ...
            for key in antex_network_proxy::brokered_credential_marker_env_keys(env) {
                if !is_valid_shell_variable_name(&key) {
                    continue;
                }
...
pub(crate) fn maybe_wrap_shell_lc_with_snapshot(
    ...
    let original_shell_is_zsh = command_uses_session_zsh
        || antex_shell_command::shell_detect::detect_shell_type(&command[0])
            == Some(ShellType::Zsh);
