...
#[cfg(unix)]
#[test_case::test_case(ApprovalsReviewer::User, antex_extension_api::ApprovalDecision::AskUser; "manual prompt")]
#[test_case::test_case(ApprovalsReviewer::AutoReview, antex_extension_api::ApprovalDecision::Allow; "cached allow")]
#[tokio::test]
async fn non_utf8_cwd_preserves_approval_routing(
    reviewer: ApprovalsReviewer,
    decision: antex_extension_api::ApprovalDecision,
) -> anyhow::Result<()> {
    ...
    struct Contributor {
        cwd: antex_utils_path_uri::LegacyAppPathString,
        decision: ApprovalDecision,
    }

    impl antex_extension_api::ApprovalReviewContributor for Contributor {
        fn decide<'a>(
            &'a self,
            input: &'a antex_extension_api::ApprovalDecisionInput<'_>,
        ) -> antex_extension_api::ExtensionFuture<'a, Option<ApprovalDecision>> {
            Box::pin(async move {
    ...
    extensions.approval_review_contributor(Arc::new(Contributor {
        cwd: antex_utils_path_uri::LegacyAppPathString::from_path_uri(&cwd, PathConvention::Posix)?,
        decision,
    }));
    ...
    *session.active_turn.lock().await = Some(crate::state::ActiveTurn::default());
    ...
    let expected = if reviewer == ApprovalsReviewer::User {
        tokio::select! {
            ...
            event = events.recv() => {
                let antex_protocol::protocol::EventMsg::ExecApprovalRequest(request) =
                    event.context("receive user prompt")?.msg
                else {
                    panic!("expected a command approval prompt");
                };
                assert_eq!(request.cwd, antex_utils_path_uri::LegacyAppPathString::from(cwd));
