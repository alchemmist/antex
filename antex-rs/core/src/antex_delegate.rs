...
pub(crate) async fn run_antex_thread_interactive(
    ...
    isolation: antex_extension_api::SessionIsolation,
    ...
    let extensions = if isolation == antex_extension_api::SessionIsolation::Isolated {
        antex_extension_api::empty_extension_registry()
    } else {
        Arc::clone(&parent_session.services.extensions)
    };
    let mut thread_extension_init = antex_extension_api::ExtensionDataInit::default();
