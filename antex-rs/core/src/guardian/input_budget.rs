...
use antex_guardian_context::HistoryTruncation;
use antex_guardian_context::RequestBudget;
use antex_guardian_context::effective_input_token_limit;
...
pub(crate) async fn check_pending(session: &Session, turn: &TurnContext) -> AntexResult<()> {
    ...
    let minimum_prefix =
        antex_protocol::protocol::TruncationPolicy::Bytes(base.text.len()).token_budget();
...
pub(crate) async fn finalize(
    ...
    let request = session.services.model_client.build_responses_request(
        ...
        /*service_tier*/ None,
        &session
            .responses_metadata(step, AntexResponsesRequestKind::Turn)
            .await,
    )?;
    ...
    let context = context
        ...
        .map_err(|error| {
            session
                ...
                .insert(super::request_budget::ExhaustedReviewBudget::Detected);
            match error {
                antex_guardian_context::SectionError::EvidenceLimitExceeded { .. } => {
                    CodexErr::ContextWindowExceeded
                }
        ...
        for (measurement, value) in cost.measurements() {
            session
                ...
                .histogram_with_boundaries(
                    antex_guardian_context::SECTION_COST_METRIC,
                    i64::try_from(value).unwrap_or(i64::MAX),
                    antex_guardian_context::SECTION_COST_BOUNDARIES,
                    &[
                        ("target", "sync"),
                        ("section", section),
