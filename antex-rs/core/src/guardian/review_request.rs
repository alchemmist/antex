...
use antex_guardian_reviewer::ReviewHost;
...
impl ReviewHost for super::super::runtime::ReviewRuntime {
    ...
    async fn servicing_turn(
        &self,
    ) -> Option<(String, Arc<antex_protocol::openai_models::ModelInfo>)> {
    ...
    async fn prepare(
    ...
    ) -> Result<(PreparedApproval, antex_guardian_reviewer::ReviewReport), ReviewDecision> {
        ...
        let (plugin_id, script_path) = plugin_attribution
            ...
            .unzip();
        let report =
            antex_guardian_reviewer::ReviewReport::new(antex_guardian_reviewer::ReviewMetadata {
    ...
    async fn record_evidence(
        &self,
        prepared: &PreparedApproval,
        event: &antex_protocol::protocol::GuardianAssessmentEvent,
    ) {
