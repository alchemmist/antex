...
use antex_protocol::models::ToolResultMetadata;
use antex_protocol::models::ToolResultSource;
...
async fn workspace_routed_http_rejects_redirects_without_a_routing_header() {
    ...
    use antex_login::WorkspaceRouting;
    use antex_login::WorkspaceRoutingRequest;
    use antex_login::WorkspaceRoutingResolver;
...
impl ModelProvider for SetupRefreshProvider {
    ...
    fn api_provider(
        &self,
    ) -> ModelProviderFuture<'_, antex_protocol::error::Result<antex_api::Provider>> {
        Box::pin(async move {
            ...
            match &self.refresh {
                ...
                SetupRefresh::ChatGpt {
                    ...
                    antex_login::auth::login_with_chatgpt_auth_tokens(
...
async fn client_setup_accepts_command_credential_refresh() {
    for routing in [
        ...
        info.auth = Some(antex_protocol::config_types::ModelProviderAuthInfo {
        ...
        assert_ne!(
            antex_model_provider::ResponsesConnectionKey::new(
                &setup.api_provider,
                setup.auth_revision
            ),
            antex_model_provider::ResponsesConnectionKey::new(
...
async fn client_setup_rebuilds_chatgpt_refresh_but_rejects_account_switches() {
    for (user, workspace, expected_calls) in [
        ...
        let initial = AntexAuth::from_external_chatgpt_tokens(
...
fn thread_responses_headers_are_scoped_to_model_and_backend_auth() {
    ...
    let chatgpt_auth = AntexAuth::create_dummy_chatgpt_auth_for_testing();
    let api_key_auth = AntexAuth::from_api_key("test-api-key");
