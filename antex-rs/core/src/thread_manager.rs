...
impl ThreadManager {
    ...
    pub async fn remove_thread_for_client(
        &self,
        thread_id: &ThreadId,
    ) -> AntexResult<Option<Arc<AntexThread>>> {
...
impl ThreadManagerState {
    ...
    async fn spawn_thread(&self, request: ThreadSpawnRequest) -> AntexResult<NewThread> {
        ...
        let isolation = thread_extension_init
            ...
            .unwrap_or_else(|| {
                if crate::guardian::is_basic_session_source(&session_source) {
                    antex_extension_api::SessionIsolation::Isolated
                } else {
                    antex_extension_api::SessionIsolation::Inherit
                }
            });
        ...
        let (instructions, inherited_exec_policy, extensions, mcp_manager, multi_agent_version) =
            if isolation == antex_extension_api::SessionIsolation::Isolated {
                (
