...
#[cfg(unix)]
use antex_network_proxy::CredentialProviderConfig;
#[cfg(unix)]
...
async fn inactive_profiles_keep_snapshots_but_active_brokers_require_sandbox() -> Result<()> {
    ...
    let session_telemetry = SessionTelemetry::new(
        ...
        "test".to_string(),
        antex_protocol::protocol::SessionSource::Cli,
    );
    let shell = Shell {
    ...
    tool_config
        .features
        .enable(antex_features::Feature::ShellSnapshot)?;
    ...
    for (config, remains_valid) in [
        ...
        proxy
            .replace_config_state(antex_network_proxy::build_config_state(
                config,
                antex_network_proxy::NetworkProxyConstraints::default(),
            )?)
            .await?;
        assert_eq!(
...
async fn snapshot_discovers_and_redacts_shell_initialized_credentials() -> Result<()> {
    ...
    assert!(!snapshot.contains("ANTEX_NETWORK_PROXY_BROKERED_CREDENTIALS"));
    ...
    fs::write(
        ...
        "test \"${HOME-missing}\" = missing && \
         test \"${ANTEX_NETWORK_PROXY_CREDENTIAL_BROKER_ACTIVE-}\" = 1\n",
    )
    .await?;
    validate_snapshot(
        &shell,
    ...
    let explicit_vendor_policy = ShellEnvironmentPolicy {
        inherit: antex_protocol::config_types::ShellEnvironmentPolicyInherit::None,
