...
use antex_utils_path::replace_path_and_deduplicate;
...
pub(crate) struct Session {
    ...
    pub(super) isolation: antex_extension_api::SessionIsolation,
    pub(crate) allowed_tools: Option<Arc<antex_extension_api::AllowedTools>>,
...
impl Session {
    ...
    pub(crate) async fn new(
        ...
        let isolation = thread_extension_init
            .get::<antex_extension_api::SessionIsolation>()
            .map(|policy| *policy)
            .unwrap_or_default();
        let allowed_tools = thread_extension_init
            .get::<antex_extension_api::AllowedTools>()
            .or_else(|| {
                ...
                crate::guardian::is_basic_session_source(&session_configuration.session_source)
                    .then(|| Arc::new(antex_guardian_reviewer::reviewer_allowed_tools()))
            });
        ...
        let session_result: anyhow::Result<Arc<Self>> = async {
            ...
            let is_worktree = session_configuration.cwd().canonicalize().ok().and_then(|cwd| {
                antex_git_utils::repository_identity(&cwd).and_then(|_| {
        ...
        mcp_elicitation_reviewer_handle: OnceLock::new(),
            ...
            let session_start_source = match &initial_history {
                InitialHistory::Forked(_) if forked_from_id.is_some() => {
                    antex_hooks::SessionStartSource::Fork
                }
                ...
                InitialHistory::Resumed(_) | InitialHistory::Forked(_) => {
                    antex_hooks::SessionStartSource::Resume
                }
                InitialHistory::New => antex_hooks::SessionStartSource::Startup,
