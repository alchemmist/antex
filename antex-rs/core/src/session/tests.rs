...
#[cfg(windows)]
use antex_network_proxy::ManagedProxyRouting;
...
fn test_model_client_session() -> crate::client::ModelClientSession {
    ...
    crate::client::ModelClient::new(
        ...
        antex_model_provider::WorkspaceRoutingContext::new(
...
async fn new_turn_refreshes_managed_network_proxy_for_sandbox_change() -> anyhow::Result<()> {
    ...
    let network_policy_decider: Arc<dyn antex_network_proxy::NetworkPolicyDecider> =
        Arc::new(|_request| async { antex_network_proxy::NetworkDecision::ask("not_allowed") });
    let (started_proxy, _) = Session::start_managed_network_proxy(
        &spec,
        &Policy::empty(),
        &initial_permission_profile,
...
async fn annotated_history_uses_explicit_model_without_a_step(
    ...
    model_info.truncation_policy = if byte_policy {
        antex_protocol::openai_models::TruncationPolicyConfig::bytes(/*limit*/ 101)
    } else {
        antex_protocol::openai_models::TruncationPolicyConfig::tokens(/*limit*/ 4)
    };
    ...
    let expected = ResponseItemEnvelope {
        item,
        metadata: retain_metadata.then_some(AntexHarnessMetadata {
    ...
    for envelope in &mut expected {
        ...
        output.body = FunctionCallOutputBody::Text(antex_utils_output_truncation::truncate_text(
            &text,
            antex_utils_output_truncation::TruncationPolicy::Tokens(expected_budget),
        ));
    }
...
async fn fork_startup_context_then_first_turn_diff_snapshot() -> anyhow::Result<()> {
    ...
    let forked = initial
        ...
        .fork_thread(
            usize::MAX,
            core_test_support::test_antex::StartThreadOptions::new(fork_config.clone()),
            rollout_path,
        )
        .await?;
...
async fn responses_metadata_uses_selected_harness_analytics_client() {
    for enabled in [true, false] {
        ...
        let metadata = session
            .responses_metadata(&step_context, AntexResponsesRequestKind::Turn)
            .await;
...
pub(crate) async fn make_session_and_context() -> (Session, TurnContext) {
    ...
    let session = Session {
        ...
        isolation: antex_extension_api::SessionIsolation::Inherit,
        allowed_tools: None,
...
async fn capability_discovery_uses_environment_permission_profile() {
    ...
    let expected_sandbox = FileSystemSandboxContext {
        ...
        windows_sandbox_selection: if cfg!(windows) {
            antex_file_system::WindowsSandboxSelection::Elevated
        } else {
            antex_file_system::WindowsSandboxSelection::Disabled
        },
    ...
    async fn run(
        ...
        session
            .interrupt_turn_with_warning(
                &ctx.sub_id,
                EventMsg::Warning(antex_protocol::protocol::WarningEvent {
...
async fn task_finish_emits_turn_item_lifecycle_for_leftover_pending_user_input() {
    ...
    image::DynamicImage::new_rgba8(/*w*/ 8, /*h*/ 8)
        ...
        .expect("encode image");
    let image_url = antex_utils_image::data_url_from_bytes("image/png", &encoded.into_inner());
