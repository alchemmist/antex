...
async fn task_finish_preserves_notification_budget_and_queued_user_input() {
    ...
    Arc::make_mut(&mut current.model_info).truncation_policy =
        antex_protocol::openai_models::TruncationPolicyConfig::tokens(/*limit*/ 400);
    ...
    let expected = vec![
        notification(antex_utils_output_truncation::truncate_text(
            &text,
            antex_utils_output_truncation::TruncationPolicy::Tokens(120),
        )),
