...
fn override_builder() -> TestAntexBuilder {
    test_antex()
...
async fn reasoning_effort_override_recovery_reuses_trusted_tail_update() -> anyhow::Result<()> {
    ...
    .await;
    let thread_settings = test.antex.restorable_thread_settings().await;
    assert_eq!(
        test.antex.suspend_turn_and_shutdown().await?,
        ...
        if matches!(startup, PrewarmStartup::Fork) {
            ...
            let forked = previous
                ...
                .fork_thread(
                    ...
                    antex_core::StartThreadOptions::new(config.clone()),
                    previous.antex.rollout_path().expect("rollout path"),
                )
                .await?;
            previous.antex = forked.thread;
