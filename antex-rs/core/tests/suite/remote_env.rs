...
async fn executor_profile_roots_survive_settings_restore_and_turn_recording() -> Result<()> {
    ...
    let test = test_antex()
        ...
        .await?;
    let runtime_roots = test.antex.config_snapshot().await.workspace_roots;
    ...
    .await?;
    let saved = test.antex.restorable_thread_settings().await;
    ...
    test.antex.restore_thread_settings(saved).await?;
    let snapshot = test.antex.config_snapshot().await;
    assert_eq!(
