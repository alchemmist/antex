...
async fn user_review_preserves_unsupported_guardian_elicitations(mode: &str) -> Result<()> {
    ...
    let test = test_antex()
        .with_config(move |config| {
            ...
            config.permissions.approval_policy = Constrained::allow_any(AskForApproval::Granular(
                antex_protocol::protocol::GranularApprovalConfig {
    ...
    match &request.request {
        antex_protocol::approvals::ElicitationRequest::Form {
            requested_schema, ..
        } => {
            assert_eq!(requested_schema, &form_schema);
        }
        antex_protocol::approvals::ElicitationRequest::Url { url, .. } => {
    ...
    test.antex
        .submit(Op::ResolveElicitation {
            ...
            decision: antex_protocol::approvals::ElicitationAction::Decline,
            content: None,
            meta: None,
        })
        .await?;
...
async fn yielded_code_mode_elicitation_keeps_live_invocation_metadata() -> Result<()> {
    ...
    let mut builder = test_antex()
        ...
        .with_config(move |config| {
            config
                .features
                .enable(antex_features::Feature::CodeMode)
                .unwrap();
            config.permissions.approval_policy = Constrained::allow_any(AskForApproval::OnRequest);
            config.approvals_reviewer = ApprovalsReviewer::User;
