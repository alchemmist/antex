...
async fn direct_call_metadata_during_compaction_respects_provider_support(
    ...
    .await;
    let compacted = test.antex.conversation_history_snapshot().await;
    ...
    match remote_compaction {
        RemoteCompactionSupport::Unsupported => assert!(compacted.iter().any(|item| {
            item["role"] == "user"
                && item["content"][0]["text"]
                    == format!("{}\n{summary}", antex_core::compact::SUMMARY_PREFIX)
        })),
    ...
    let captured = test.antex.conversation_history_snapshot().await;
