...
async fn background_network_approval_uses_current_review_settings_and_original_execution(
    ...
    .await?;
    let EventMsg::TurnStarted(first_turn) = wait_for_event(&test.antex, |event| {
...
PY"#
    ...
    assert!(test.antex.list_background_terminals().await.is_empty());
...
async fn unattributed_network_request_uses_active_turn_environment_fallback(
    ...
    if approvals_reviewer == ApprovalsReviewer::AutoReview {
        ...
        let thread_id = antex_protocol::ThreadId::from_string(thread_id)?;
        ...
        tokio::time::timeout(Duration::from_secs(5), test.antex.shutdown_and_wait())
            .await
...
async fn thread_turnover_closes_managed_proxy_tunnels() -> Result<()> {
    ...
    let mut network = NetworkProxyConfig {
        enabled: true,
        mode: antex_network_proxy::NetworkMode::Full,
...
pub(super) fn guardian_parent_catalog() -> ModelsResponse {
    let template = antex_models_manager::bundled_models_response()
        ...
        models: ["guardian-parent-a", "guardian-parent-b"]
            ...
            .map(|slug| {
                ...
                model.supported_reasoning_levels.retain(|level| {
                    level.effort != antex_protocol::openai_models::ReasoningEffort::Low
                });
                model
                    .model_messages
                    .as_mut()
...
fn managed_network_unified_exec_builder(features: &[Feature]) -> Result<TestAntexBuilder> {
