...
use antex_extension_api::AllowedTools;
...
async fn dropping_startup_cleans_up_while_required_mcp_is_stalled() -> anyhow::Result<()> {
    ...
    let fixture = test_antex().build_with_auto_env(&server).await?;
    ...
    options.environments = Some(fixture.antex.environment_selections().await);
    options
...
async fn owner_cancellation_closes_agent_and_preserves_history_and_parent() -> anyhow::Result<()> {
    ...
    duplicate.environments = Some(fixture.antex.environment_selections().await);
    duplicate
        .thread_extension_init
        .insert(SessionIsolation::Isolated);
    ...
    assert!(history.items.iter().any(|item| matches!(
        item,
        antex_history::RolloutItem::EventMsg(EventMsg::TurnComplete(_))
    )));
...
async fn startup_allowlist_controls_advertising_and_execution(
    ...
    let fixture = test_antex()
        .with_config(|config| {
            ...
            config
                .features
                .disable(antex_features::Feature::CodeMode)
