...
async fn guardian_subagent_review_preserves_late_root_user_authorization(
    ...
    if matches!(root_context, RootContext::Migrating) {
        ...
        test.antex = super::guardian_checkpoint_migration::resume(
            &test,
            &test.antex,
        ...
        .await?;
        assert_eq!(
            antex_core::context::GuardianContextMode::from_history(
                test.antex.conversation_history_snapshot().await.as_ref()
            ),
            antex_core::context::GuardianContextMode::Legacy,
        );
    }
