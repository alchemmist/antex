...
async fn finish_turn(thread: &AntexThread) {
...
async fn saved_history(test: &TestAntex, thread: &AntexThread) -> Result<Vec<RolloutItem>> {
...
pub(super) async fn migration_scenario() -> Result<Vec<responses::ResponsesRequest>> {
    ...
    test.antex.append_rollout_items(&history).await?;
    let thread = resume(&test, &test.antex, history).await?;
    assert_eq!(
