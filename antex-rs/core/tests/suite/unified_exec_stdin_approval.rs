...
async fn harness(builder: TestAntexBuilder) -> Result<TestAntexHarness> {
    TestAntexHarness::with_auto_env_builder(builder.with_config(|config| {
        for feature in [
...
async fn stdin_reviews_retained_grants_after_turn_permissions_expire() -> Result<()> {
    ...
    skip_if_sandbox!(Ok(()));
    let harness = harness(test_antex()).await?;
...
async fn strict_stdin_review_reaches_guardian_with_sandbox_prompts_disabled() -> Result<()> {
    ...
    impl antex_extension_api::ApprovalReviewContributor for ReviewedCalls {
        fn decide<'a>(
            &'a self,
    ...
    let harness = harness(test_antex().with_extensions(Arc::new(extensions.build()))).await?;
    let test = harness.test();
