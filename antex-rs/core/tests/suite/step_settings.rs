...
fn direct_tool_settings_test() -> TestAntexBuilder {
...
async fn tool_result_history_keeps_originating_model_across_switch_and_replay() -> Result<()> {
    ...
    for call_id in ["call-a", "call-b"] {
        let call = wait_for_event_match(&test.antex, |event| match event {
    ...
    let paused_request = wait_for_event_match(&test.antex, |event| match event {
        EventMsg::RequestUserInput(request) => Some(request.clone()),
        _ => None,
    })
    .await;
    ...
    .await?;
    answer_paused_turn(&test.antex, &paused_request.turn_id).await?;
    ...
    async fn saved_outputs(path: &std::path::Path) -> Result<Vec<(Value, Option<usize>)>> {
        let history = antex_rollout::RolloutRecorder::get_rollout_history(path).await?;
        Ok(history
            .get_rollout_items()
            .iter()
    ...
    for model in [MODEL_A, MODEL_B] {
        ...
        let resumed = test
            ...
            .resume_thread_from_rollout(
                replay_config.clone(),
                rollout_path.clone(),
                antex_core::test_support::auth_manager_from_auth(AntexAuth::from_api_key("dummy")),
                /*parent_trace*/ None,
...
async fn custom_tool_output_replay_preserves_originating_budget() -> Result<()> {
    ...
    test.antex.shutdown_and_wait().await?;
    let rollout_path = test.antex.rollout_path().expect("rollout path");
    let history = antex_rollout::RolloutRecorder::get_rollout_history(&rollout_path).await?;
    let saved = history
        .get_rollout_items()
        .iter()
    ...
    let test = direct_tool_settings_test()
        .with_config(|config| {
            config
                ...
                .expect("test permissions");
            for model in &mut config.model_catalog.as_mut().expect("models").models {
                model.input_modalities = vec![antex_protocol::openai_models::InputModality::Text];
                model.supports_image_detail_original = false;
                if model.slug == MODEL_B {
                    model
                        .input_modalities
                        .push(antex_protocol::openai_models::InputModality::Image);
    ...
    let test = direct_tool_settings_test()
        .with_config(|config| {
            ...
            for model in &mut config.model_catalog.as_mut().expect("models").models {
                ...
                model.truncation_policy = antex_protocol::openai_models::TruncationPolicyConfig {
                    mode: antex_protocol::openai_models::TruncationMode::Bytes,
...
async fn captured_step_controls_mcp_output_limit(supports_images: bool) -> Result<()> {
    ...
    let test = direct_tool_settings_test()
        ...
        .build_with_auto_env(&server)
        .await?;
    wait_for_mcp_server(&test.antex, "calendar").await?;
    let paused = start_paused_turn(&test.antex).await?;
    apply_turn_settings(
        &test.antex,
        &paused.turn_id,
    ...
    .await?;
    answer_paused_turn(&test.antex, &paused.turn_id).await?;
    let recorded_output = wait_for_event_match(&test.antex, |event| match event {
...
impl<'call> ToolExecutor<ToolCall<'call>> for SettingsEcho {
    ...
    fn spec(&self) -> ToolSpec {
        ToolSpec::Function(antex_tools::ResponsesApiTool {
...
async fn captured_step_controls_mcp_resource_output() -> Result<()> {
    ...
    let test = direct_tool_settings_test()
        ...
        .build_with_auto_env(&server)
        .await?;
    wait_for_mcp_server(&test.antex, "resources").await?;
    let paused = start_paused_turn(&test.antex).await?;
    apply_turn_settings(
        &test.antex,
        &paused.turn_id,
