...
fn result_metadata_apps_builder(base_url: String, account_email: &str) -> TestAntexBuilder {
...
async fn result_metadata_follows_call_binding(
    ...
    let captured = antex_core::test_support::history_with_tool_call_metadata(&test.antex).await;
    if direct {
        let result = captured
            .iter()
            .find_map(|item| match item {
                antex_protocol::models::ResponseItem::FunctionCallOutput {
                    call_id,
                    output,
                    ..
    ...
    for (call_id, enabled) in [("call-off", false), ("call-on", true)] {
        if enabled {
            ...
            config
                ...
                .unwrap();
            test.antex.refresh_runtime_config(config).await;
...
struct ResponseIdObserver {
    ...
    originating_items: Mutex<Vec<(String, Option<antex_protocol::ResponseItemId>)>>,
