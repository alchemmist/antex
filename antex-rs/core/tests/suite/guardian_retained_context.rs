...
async fn wait_for_thread_idle(thread: &AntexThread) {
...
async fn standalone_fork_retains_inherited_user_instructions(
    ...
    let fork = if history_mode == ThreadHistoryMode::Paginated {
        ...
        test.thread_manager
            .fork_prepared_thread(
                antex_core::StartThreadOptions::new(test.config.clone()),
                prepared,
            )
            .await?
    } else {
    ...
    let answers = antex_guardian_context::render_verified_answers(retained);
    assert_eq!(
        (answers.fragments, answers.complete),
        (
            vec![antex_guardian_context::GuardianRootMessage::IncompleteVerifiedAnswers.render()],
            false
        ),
    );
    assert_eq!(
        ...
        vec![
            ...
            (
                antex_guardian_context::truncate_text(&local_instruction, /*max_tokens*/ 900),
                !compacted,
            ),
        ],
    );
...
async fn forked_parent_instructions_do_not_become_local_authorization(
    ...
    .await;
    wait_for_thread_idle(&test.antex).await;
    if compact_parent {
