...
async fn submit_turn_with_trigger(test: &TestAntex, prompt: &str, trigger: &str) -> Result<()> {
    test.antex
        .start_or_steer_turn(
            TurnInputRequest::user_input(vec![UserInput::Text {
...
async fn spawned_full_history_v2_child_uses_model_precedence_without_dropping_context(
    ...
    let mut builder = test_antex().with_config(move |config| {
        ...
        if matches!(selection, FullHistoryV2ModelSelection::CurrentTimeReminders) {
            ...
            config.current_time_reminder = Some(CurrentTimeReminderConfig {
                ...
                clock_source: antex_features::CurrentTimeSource::External,
    ...
    if matches!(selection, FullHistoryV2ModelSelection::CurrentTimeReminders) {
        ...
        impl antex_core::TimeProvider for FailFirstClockRead {
            fn current_time(&self, _thread_id: ThreadId) -> antex_core::TimeFuture<'_> {
