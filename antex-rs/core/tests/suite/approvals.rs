...
async fn shell_startup_credentials_are_brokered(
    ...
    let builder = if shell_mode == "zsh_fork" {
        ...
        let Some(zsh) = antex_core::shell::get_shell(antex_core::shell::ShellType::Zsh) else {
            return Ok(());
        };
        test_antex()
            .with_user_shell(zsh)
...
"#,
    ...
    if direct && !allow_login_shell {
        let Some(bash) = antex_core::shell::get_shell(antex_core::shell::ShellType::Bash) else {
            return Ok(());
        };
    ...
    let command = r#"printf '%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n' "$GH_HOST" "$GH_ENTERPRISE_TOKEN" "$CUSTOM_HOST" "$CUSTOM_API_KEY" "$ANTEX_NETWORK_PROXY_CREDENTIAL_BROKER_ACTIVE" "$PWD" "$BROKERED_STARTUP_CWD" "${LOGIN_SNAPSHOT_READY-unset}" "$AUTH_HEADER""#;
    let command = format!(
...
async fn brokered_posix_startup_preserves_application_env(credential_startup: bool) -> Result<()> {
    ...
    let Some(shell) = antex_core::shell::get_shell(antex_core::shell::ShellType::Sh) else {
        return Ok(());
    };
    let home = Arc::new(TempDir::new()?);
...
"#,
    ...
    let test = test_antex()
        ...
        .build(&server)
        .await?;
    let script = r#"printf '%s\n%s\n%s\n%s\n' "$GH_TOKEN" "$ANTEX_NETWORK_PROXY_CREDENTIAL_BROKER_ACTIVE" "${ENV-unset}" "$CORP_REGION""#;
    let command = format!(
        "{} 2>/dev/null",
