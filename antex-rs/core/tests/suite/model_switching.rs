...
async fn first_turn_model_change_appends_model_instructions_developer_message(
    ...
    let mut builder =
        test_antex().with_model_info_override("gpt-5.6-terra", configure_model_switching_fixture);
...
async fn model_change_projects_media_without_changing_live_or_replayed_history(
    ...
    let image_url = antex_utils_image::data_url_from_bytes("image/png", &png.into_inner());
    ...
    let thread = match source {
        MediaHistorySource::Live => Arc::clone(&test.antex),
