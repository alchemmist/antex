...
async fn guardian_reused_reviewer_avoids_stale_catalog_lookup() -> Result<()> {
    ...
    let bundled = antex_models_manager::bundled_models_response()?;
...
async fn uses_cache_when_version_matches() -> Result<()> {
    ...
    let mut builder = test_antex().with_auth(AntexAuth::create_dummy_chatgpt_auth_for_testing());
    let identity = antex_model_provider::test_support::models_cache_entry(
        &antex_model_provider_info::ModelProviderInfo::create_openai_provider(Some(format!(
            "{}/v1",
            server.uri()
        ))),
