...
#[test_case(AntexAuth::create_dummy_chatgpt_auth_for_testing(), "OpenAI", "/backend-api/codex", false, "/backend-api/codex/responses", true; "chatgpt_marks_guardian_by_default")]
#[test_case(AntexAuth::create_dummy_chatgpt_auth_for_testing(), "OpenAI", "/backend-api/codex", true, "/backend-api/codex/responses", true; "legacy_opt_in_still_accepted")]
...
#[test_case(AntexAuth::create_dummy_chatgpt_auth_for_testing(), "OpenAI", "/backend-api/codex", true, "/backend-api/codex/responses", false; "retry_without_response_id_keeps_last_parent")]
async fn guardian_session_inherits_parent_http_fallback(
    ...
    let credits_enabled =
        auth.uses_antex_backend() && provider_name == "OpenAI" && base_path == "/backend-api/codex";
...
async fn guardian_session_prewarms_and_is_reused_for_first_review(
    ...
    let credits_enabled = uses_antex_backend;
    ...
    assert!(
        test.antex
            ...
            .get::<antex_guardian_reviewer::ReviewerTasks>()
    ...
    assert!(matches!(
        ...
        Err(antex_thread_store::ThreadStoreError::ThreadNotFound { .. })
    ));
...
async fn guardian_reviews_delayed_and_new_actions_after_catalog_refresh() -> Result<()> {
    ...
    let test = test_antex()
        ...
        .with_config(|config| {
            ...
            config.model_reasoning_effort =
                Some(antex_protocol::openai_models::ReasoningEffort::High);
            config.model_reasoning_summary =
                Some(antex_protocol::config_types::ReasoningSummary::Detailed);
        })
        .build_with_auto_env(&server)
        .await?;
    ...
    test.antex
        .submit(Op::TurnSettings {
            turn_id,
            update: antex_protocol::protocol::TurnSettingsUpdate {
                model: Some("guardian-parent-b".to_string()),
                effort: Some(Some(antex_protocol::openai_models::ReasoningEffort::Medium)),
                summary: Some(antex_protocol::config_types::ReasoningSummary::Concise),
                ..Default::default()
            },
            reply,
        })
...
async fn guardian_timeout_rejects_tool_call_with_acting_model_instructions(
    ...
    impl antex_extension_api::ApprovalReviewContributor for TimedOutReviewContributor {
        fn decide<'a>(
            &'a self,
            input: &'a antex_extension_api::ApprovalDecisionInput<'_>,
        ) -> antex_extension_api::ExtensionFuture<'a, Option<antex_extension_api::ApprovalDecision>>
        {
            assert_eq!(input.tool_call_id, Some("exec-call-timed-out"));
            Box::pin(async {
                Some(antex_extension_api::ApprovalDecision::Reviewed(
...
impl antex_extension_api::ApprovalReviewContributor for AttemptCachedApproval {
    fn decide<'a>(
        &'a self,
    ...
    ) -> antex_extension_api::ExtensionFuture<'a, Option<antex_extension_api::ApprovalDecision>>
    {
        ...
        Box::pin(async { Some(antex_extension_api::ApprovalDecision::Allow) })
    }
}
...
async fn yielded_code_mode_denials_interrupt_the_servicing_turn() -> Result<()> {
    ...
    let mut builder = test_antex()
        .with_model_info_override("gpt-5.4", |model| {
            model.tool_mode = Some(antex_protocol::openai_models::ToolMode::CodeMode);
...
"#
    ...
    loop {
        let event =
            tokio::time::timeout(Duration::from_secs(60), test.antex.next_event()).await??;
        match event.msg {
