...
use antex_extension_api::LoadInstructionsFuture;
use antex_extension_api::LoadedUserInstructions;
use antex_extension_api::ThreadInstructionsProvider;
...
struct WarningInstructionsProvider {
    inner: AntexHomeUserInstructionsProvider,
...
async fn persisted_resume_history(
    thread: &Arc<antex_core::AntexThread>,
...
-> Result<()> {
    ...
    failed to load AGENTS.md instructions for environment `local`: failed to prepare fs sandbox: failed to prepare Seatbelt sandbox: writable root $ANTEX_HOME/visualizations contains symlink component $ANTEX_HOME/visualizations; symlinked writable roots are not supported.
...
-> Result<()> {
    ...
    let provider = Arc::new(WarningInstructionsProvider {
        inner: AntexHomeUserInstructionsProvider::new(home.path().to_path_buf().abs()),
    ...
    let EventMsg::Error(error) = wait_for_event(&test.antex, |event| {
...
async fn fork_injects_changed_agents_md_once() -> Result<()> {
    ...
    let forked = parent
        ...
        .fork_thread(
            ...
            antex_core::StartThreadOptions::new(fork_config),
            rollout_path,
        )
        .await?;
