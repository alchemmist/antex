...
async fn remote_resume_restores_saved_server_profile_without_permission_overrides() -> Result<()> {
    ...
    server
        .thread_settings_update(antex_app_server_protocol::ThreadSettingsUpdateParams {
            thread_id: thread_id.to_string(),
            permissions: Some("server-only".into()),
            approval_policy: Some(antex_app_server_protocol::AskForApproval::Never),
            approvals_reviewer: Some(antex_app_server_protocol::ApprovalsReviewer::AutoReview),
            ..Default::default()
        })
        .await?;
    tokio::time::timeout(std::time::Duration::from_secs(5), async {
        loop {
            if let Some(antex_app_server_client::AppServerEvent::ServerNotification(notification)) =
                server.next_event().await
                && let antex_app_server_protocol::ServerNotification::ThreadSettingsUpdated(
                    settings,
                ) = *notification
    ...
    assert_eq!(
        ...
        antex_protocol::config_types::ApprovalsReviewer::AutoReview
    );
    ...
    let explicit_config = ConfigBuilder::default()
        .codex_home(home.path().to_path_buf())
        .harness_overrides(crate::legacy_core::config::ConfigOverrides {
            sandbox_mode: Some(antex_protocol::config_types::SandboxMode::ReadOnly),
            approval_policy: Some(antex_protocol::protocol::AskForApproval::OnRequest),
            ..Default::default()
        })
        .build()
        .await?;
    ...
    assert_eq!(
        ...
        antex_app_server_protocol::AskForApproval::OnRequest
    );
    assert_eq!(
        explicit_fork.session.permission_profile,
        antex_protocol::models::PermissionProfile::read_only()
    );
    server.shutdown().await?;
    Ok(())
}
...
async fn cached_legacy_resume_revalidates_history_across_migration_settings() -> Result<()> {
        ...
        [(false, false), (false, true), (true, false), (true, true)]
    {
        ...
        let legacy = {
            ...
            assert!(
                antex_rollout::try_acquire_rollout_maintenance_lock(codex_home.path())?.is_none()
            );
            resume.await?
        };
        assert_eq!(app_server.next_request_id, next_request_id + 2);
