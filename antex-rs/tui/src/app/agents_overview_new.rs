...
pub(crate) struct PendingWorktree {
    pub(in crate::app) manager: antex_worktree::WorktreeManager,
    pub(in crate::app) checkout: Option<antex_worktree::ManagedWorktree>,
}
    ...
    pub(in crate::app) async fn new_agents_overview_worktree(
        ...
        let setup = async {
            ...
            let host = crate::legacy_core::config::load_config_toml_with_layer_stack(
                ...
                /*cwd*/ None,
                Vec::new(),
                antex_config::ConfigLoadOptions::default(),
            )
            .await?;
            let settings = antex_worktree::WorktreeSettings::for_cli(
                &self.config.codex_home,
                host.config_toml.desktop.as_ref(),
            )?;
            anyhow::Ok(antex_worktree::WorktreeManager::new(settings))
        }
        .await;
        ...
        tokio::spawn(async move {
            let result = tokio::task::spawn_blocking(move || {
                antex_worktree::default_worktree_base(config.cwd.as_path())
                    .and_then(|base| {
                        manager.create(&antex_worktree::CreateWorktree {
