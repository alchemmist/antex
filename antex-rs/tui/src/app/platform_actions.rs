...
pub(super) async fn windows_sandbox_ready(app_server: &mut AppServerSession) -> bool {
    ...
    matches!(
        ...
        .await,
        Ok(Ok(
            antex_app_server_protocol::WindowsSandboxReadinessResponse {
                status: antex_app_server_protocol::WindowsSandboxReadiness::Ready,
            }
        ))
    )
}
...
impl App {
        ...
        let response = tokio::time::timeout(
            ...
            app_server
                ...
                .request_typed(ClientRequest::WindowsSandboxSetupStart {
                    request_id,
                    params: antex_app_server_protocol::WindowsSandboxSetupStartParams {
                        mode: setup_mode,
    ...
    pub(super) fn finish_windows_sandbox_setup_start(
        ...
        response: std::result::Result<
            std::result::Result<
                antex_app_server_protocol::WindowsSandboxSetupStartResponse,
                TypedRequestError,
            >,
        ...
        match response {
            Ok(Ok(antex_app_server_protocol::WindowsSandboxSetupStartResponse {
                started: true,
            })) => {}
            Err(_) => {
