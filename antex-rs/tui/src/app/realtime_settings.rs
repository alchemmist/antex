...
fn configured_voice(
    config: &antex_app_server_protocol::Config,
        ...
        .await
        {
            Ok(response) => {
                ...
                let effective = crate::config_update::read_effective_config_if_supported(
                    ...
                    None if response.status == antex_app_server_protocol::WriteStatus::OkOverridden => Err(color_eyre::eyre::eyre!("the saved voice is overridden, but this server cannot read effective settings")),
                    None => Ok(Some(voice)),
                });
                match effective {
                    Ok(effective_voice) => {
