...
async fn delegated_core_events_keep_private_output_hidden_and_deliver_final_speech() -> Result<()> {
        ...
        loop {
            tokio::select! {
                ...
                op = ops.recv() => {
                    ...
                    app.handle_event(&mut tui, &mut app_server, AppEvent::AntexOp(op)).await?;
                }
            }
        }
    })
    ...
    app.handle_event(&mut tui, &mut app_server, AppEvent::AntexOp(speech))
        .await?;
    assert_eq!(
        timeout(
    ...
    app.handle_event(&mut tui, &mut app_server, AppEvent::AntexOp(typed_op))
        .await?;
