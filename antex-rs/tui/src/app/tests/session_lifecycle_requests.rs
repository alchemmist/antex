...
async fn managed_worktree_transitions_bind_owner_and_preserve_only_fork_history() -> Result<()> {
    ...
    fs::create_dir_all(source.join(".antex"))?;
...
"#,
    ...
    fs::write(source.join(".antex/config.toml"), destination)?;
...
"#,
    ...
    let manager = antex_worktree::WorktreeManager::new(
        antex_worktree::WorktreeSettings::for_cli(&home, /*desktop*/ None)
            .map_err(|error| color_eyre::eyre::eyre!(error.to_string()))?,
    );
...
"#,
    ...
    for mode in [ManagedWorktreeMode::New, ManagedWorktreeMode::Fork] {
        ...
        let metadata = antex_rollout::read_session_meta_line(&rollout).await?;
    ...
    let unowned = manager
        .create(&antex_worktree::CreateWorktree {
    ...
    antex_rollout::spawn_rollout_compression_worker(home.clone());
...
async fn command_center_read_only_open_requests_and_failure_preservation() -> Result<()> {
    for (history_capabilities, saved_turn_count) in [
        ...
        let (mut app, _antex_home) = Box::pin(make_history_test_app()).await?;
        std::fs::write(
                ...
                let events = [
                    ...
                    EventMsg::ItemCompleted(ItemCompletedEvent {
                        ...
                        item: TurnItem::AgentMessage(AgentMessageItem {
                            ...
                            phase: Some(antex_protocol::models::MessagePhase::FinalAnswer),
                            memory_citation: None,
                            delivery: None,
                            questions: None,
                        }),
