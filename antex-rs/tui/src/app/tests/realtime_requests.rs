...
async fn check_remote_voice_start(
    ...
    Box::pin(app.handle_event(
        ...
        AppEvent::PersistRealtimeVoiceSelection {
            voice: antex_protocol::protocol::RealtimeVoice::Juniper,
        },
    ))
    .await?;
    if capabilities == super::session_lifecycle_requests::HistoryCapabilities::Current {
        assert_eq!(
            ...
            (
                Some(antex_protocol::protocol::RealtimeVoice::Juniper),
                Some(antex_protocol::protocol::RealtimeVoice::Juniper)
            )
        );
    }
    ...
    app.config.realtime.voice = Some(antex_protocol::protocol::RealtimeVoice::Maple);
    app.chat_widget
        .on_realtime_voice_saved(antex_protocol::protocol::RealtimeVoice::Maple);
    Box::pin(app.handle_event(
        &mut tui,
        &mut app_server,
        AppEvent::AntexOp(Op::RealtimeConversationStart {
            thread_id,
...
async fn switching_agent_threads_stops_backend_voice_once_through_app_server() -> Result<()> {
    ...
    app.enqueue_thread_notification(
        ...
        ServerNotification::ThreadRealtimeTranscriptDone(
            antex_app_server_protocol::ThreadRealtimeTranscriptDoneNotification {
                thread_id: source.to_string(),
                role: "user".into(),
...
async fn switching_threads_keeps_the_source_voice_partial_only_on_reattach() {
    ...
    app.chat_widget.handle_server_notification(
        ServerNotification::ThreadRealtimeTranscriptDelta(
            antex_app_server_protocol::ThreadRealtimeTranscriptDeltaNotification {
                thread_id: source.to_string(),
                role: "user".into(),
...
async fn rejected_realtime_speech_restores_the_delegated_final_answer() -> Result<()> {
    ...
    let mut tui = crate::tui::test_support::make_test_tui()?;
    Box::pin(app.handle_event(&mut tui, &mut app_server, AppEvent::AntexOp(speech))).await?;
    assert_eq!(
...
async fn delegated_final_speech_reaches_app_server_once_and_stale_speech_is_rejected() -> Result<()>
{
    ...
    assert!(ops.try_recv().is_err());
    let mut tui = crate::tui::test_support::make_test_tui()?;
    Box::pin(app.handle_event(&mut tui, &mut app_server, AppEvent::AntexOp(speech.clone())))
        .await?;
    assert!(!app.chat_widget.has_pending_realtime_speech(delivery_id));
    assert_eq!(
    ...
    Box::pin(app.handle_event(&mut tui, &mut app_server, AppEvent::AntexOp(queued))).await?;
    assert_eq!(
        recorded_params(&requests, "thread/realtime/appendSpeech").len(),
        1
    );
...
async fn remote_voice_picker_reads_server_preference() -> Result<()> {
    ...
    app.chat_widget
        .set_realtime_voice(Some(antex_protocol::protocol::RealtimeVoice::Maple));
...
async fn overridden_voice_save_keeps_effective_voice() -> Result<()> {
    for project_override in [false, true] {
        ...
        let client = crate::start_embedded_app_server(
            ...
            app.config.clone(),
            overrides,
            antex_config::LoaderOverrides::without_managed_config_for_tests(),
            /*strict_config*/ false,
        ...
        while events.try_recv().is_ok() {}
        app.persist_realtime_voice(&server, antex_protocol::protocol::RealtimeVoice::Juniper)
            .await;
        assert_eq!(
            (
                app.config.realtime.voice,
            ...
            (
                Some(antex_protocol::protocol::RealtimeVoice::Maple),
                Some(antex_protocol::protocol::RealtimeVoice::Maple)
            ),
        );
