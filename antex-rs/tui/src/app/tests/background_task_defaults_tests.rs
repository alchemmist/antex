...
fn trust_launch_folder(app: &mut App) {
    ...
    app.config.active_project.trust_level = Some(antex_protocol::config_types::TrustLevel::Trusted);
}
...
async fn command_center_new_reads_server_defaults_for_actual_destination() -> Result<()> {
    ...
    for (mode, explicit_cwd, launch_override, expected_cwd, expected_model) in [
        ...
        std::fs::create_dir(destination.path().join(".antex"))?;
        std::fs::write(
            destination.path().join(".antex/config.toml"),
    ...
    for _ in 0..2 {
        ...
        assert_eq!(
            ...
            (
                ...
                antex_protocol::protocol::AskForApproval::UnlessTrusted,
    ...
    for attempt in 0..3 {
        ...
        assert_eq!(
            ...
            (
                ...
                antex_protocol::protocol::AskForApproval::OnRequest,
            )
        );
        if attempt == 1 {
            app.agents_overview
