...
See the Antex keymap documentation for supported actions and examples."
        ...
        let _ =
            app.initialize_server_version_notice(ANTEX_CLI_VERSION, app_server.server_version());
            ...
            loop {
                ...
                let control = select! {
                    ...
                    result = async { match reconnect.as_mut() { Some(future) => future.await, None => std::future::pending().await } }, if reconnect.is_some() && !has_pending_app_events => {
                        ...
                        match result {
                            Ok(connected) => {
                                app.finish_reconnect(tui, &mut app_server, &mut app_event_rx, connected, ANTEX_CLI_VERSION).await?;
