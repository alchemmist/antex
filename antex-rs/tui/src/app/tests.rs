...
async fn resume_rejects_selected_config_profile_permission_definitions() -> Result<()> {
    ...
    let mut loader = antex_config::LoaderOverrides::without_managed_config_for_tests();
...
async fn inactive_thread_started_notification_initializes_replay_session() -> Result<()> {
    ...
    app.enqueue_thread_notification(
        ...
        ServerNotification::ThreadStarted(ThreadStartedNotification {
            thread: Thread {
                originator: None,
                environments: Some(vec![antex_app_server_protocol::ThreadEnvironment {
...
async fn thread_read_session_state_does_not_reuse_primary_permission_profile() {
    ...
    let model_line = header
        .lines()
        .find(|line| line.contains("server-reported-model"))
