...
impl App {
    ...
    pub(super) async fn refresh_in_memory_config_from_disk(&mut self) -> Result<()> {
        ...
        self.refresh_server_version_overview_notice(ANTEX_CLI_VERSION);
    ...
    pub(super) async fn verify_windows_sandbox_mode_after_setup(
        &mut self,
        app_server: &mut AppServerSession,
        requested_mode: antex_app_server_protocol::WindowsSandboxSetupMode,
    ) -> bool {
...
mod tests {
    ...
    async fn advanced_reasoning_keeps_configured_defaults_for_new_threads() {
        ...
        let new_thread_config = app.config.clone();
