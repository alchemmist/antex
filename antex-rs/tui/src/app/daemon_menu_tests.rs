...
async fn daemon_menu_is_read_only_and_confirmation_can_cancel_or_handoff() {
    ...
    std::fs::write(package.path().join("bin/antex"), "CLI").unwrap();
    std::fs::write(package.path().join("antex-package.json"), "{}").unwrap();
    app.daemon_cli_executable =
        Some(AbsolutePathBuf::from_absolute_path(package.path().join("bin/antex")).unwrap());
    ...
    insta::assert_snapshot!(overview.lines().next().unwrap().trim(), @"Service v0.153.0 < Antex CLI v0.154.0 · /daemon");
    ...
    for (width, source, snapshot) in [
        ...
        app.daemon_cli_executable = Some(
            AbsolutePathBuf::from_absolute_path(if cfg!(windows) {
                r"C:\cli-build\bin\antex"
            } else {
                "/x/cli-build/bin/antex"
            })
            .unwrap(),
        );
        ...
        insta::assert_snapshot!(
            ...
            render_bottom_popup(&app.chat_widget, width)
                .replace(r"C:\cli-build\bin\antex", "/x/cli-build/bin/antex")
        );
    ...
    std::fs::remove_file(package.path().join("antex-package.json")).unwrap();
    // The full CLI can have any filename; capability comes from its entry point.
    app.daemon_cli_executable =
        Some(AbsolutePathBuf::from_absolute_path(package.path().join("bin/antex-tui")).unwrap());
    app.open_daemon_menu();
    insta::assert_snapshot!(
