...
pub(crate) fn fetch(
    ...
    tokio::spawn(async move {
        let result = async {
            ...
            let archive_root = codex_home.join(antex_rollout::ARCHIVED_SESSIONS_SUBDIR);
...
pub(crate) async fn list(codex_home: PathBuf, cwd: PathBuf) -> anyhow::Result<Vec<Entry>> {
    ...
    .await?;
    let settings =
        antex_worktree::WorktreeSettings::for_cli(&codex_home, host.config_toml.desktop.as_ref())?;
    // Closing the popup discards its result; an already-running blocking Git call still finishes.
    tokio::task::spawn_blocking(move || {
        let cwd = antex_git_utils::get_git_repo_root(&cwd).unwrap_or(cwd);
        let manager = antex_worktree::WorktreeManager::new(settings);
        Ok(manager
            .list(&cwd)?
            .into_iter()
            .map(|checkout| Entry {
...
pub(crate) async fn remove(
    ...
    let settings =
        antex_worktree::WorktreeSettings::for_cli(&codex_home, host.config_toml.desktop.as_ref())?;
    tokio::task::spawn_blocking(move || {
        antex_worktree::WorktreeManager::new(settings).remove(&source_cwd, &root)
    })
    .await?
}
