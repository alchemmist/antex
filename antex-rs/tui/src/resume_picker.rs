...
async fn run_resume_picker_with_launch_context(
    ...
    let options = SessionPickerRunOptions {
        ...
        local_filter_cwd,
        worktrees_enabled: config.features.enabled(antex_features::Feature::Worktrees),
                ...
                PickerLoadRequest::Page(request) => {
                    let cwd_filter = page_cwd_filter.for_request(
                        ...
                        config.as_ref().is_some_and(|config| {
                            config.features.enabled(antex_features::Feature::Worktrees)
                        }),
                    );
    ...
    fn row_matches_local_cwd(&self, row: &Row) -> bool {
        ...
        paths_match(row_cwd, filter_cwd)
            || (self.worktrees_enabled
                && antex_git_utils::repository_identity(row_cwd)
                    .zip(antex_git_utils::repository_identity(filter_cwd))
...
pub(crate) fn repository_cwd_filter(
    ...
    if worktrees_enabled
        && !uses_remote_filesystem
        && let Some(cwds) = antex_git_utils::linked_worktree_cwds(cwd)
        && cwds.len() > 1
    {
...
mod tests {
    ...
    async fn worktrees_filtering_requires_feature() {
        ...
        config
            .features
            .set_enabled(antex_features::Feature::Worktrees, /*enabled*/ true)
            .unwrap();
