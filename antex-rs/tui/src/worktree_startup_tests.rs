...
async fn refreshed_bundle_rechecks_source_during_config_reload() -> anyhow::Result<()> {
    ...
    let manager = antex_worktree::WorktreeManager::new(antex_worktree::WorktreeSettings::for_cli(
        dir.path(),
        /*desktop*/ None,
    )?);
    let checkout = antex_worktree::ManagedWorktree {
    ...
    .await?;
    let refreshed =
        antex_config::test_support::CloudConfigBundleFixture::loader_with_enterprise_config(
            format!(
                "[projects.{}]\ntrust_level = \"untrusted\"\n",
                serde_json::to_string(&antex_config::loader::project_trust_key(&nested))?
            ),
        );
