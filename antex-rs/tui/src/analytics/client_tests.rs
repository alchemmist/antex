...
pub(in crate::analytics) async fn live(
    ...
    config.cli_auth_credentials_store_mode = antex_login::AuthCredentialsStoreMode::File;
...
async fn analytics_requests_use_reloaded_credentials_for_the_same_identity() {
        ...
        .request(|client| async move {
            client
                .get_account_analytics(
                    antex_backend_client::AnalyticsReport::Usage,
                    "2026-09-01",
                    "2026-09-07",
                )
                .await
    ...
    assert_eq!(
        result,
        antex_backend_client::AnalyticsResponse::Usage(
            antex_backend_client::analytics_models::DailyProductSurfaceUsageResponse::default()
        )
    );
    server.verify().await;
}
