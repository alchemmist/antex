...
pub(crate) struct ManagedTuiWorktree {
    manager: antex_worktree::WorktreeManager,
    checkout: antex_worktree::ManagedWorktree,
...
async fn latest_thread_cwd(path: Option<PathBuf>, fallback: PathBuf) -> PathBuf {
    ...
    tokio::task::spawn_blocking(move || {
        ...
        while let Some(outcome) = scanner.scan_next_rollout_line().ok()? {
            if let antex_rollout::ScanOutcome::Parsed(antex_rollout::RolloutLine {
                item: antex_rollout::RolloutItem::TurnContext(item),
                ..
            }) = outcome
            {
...
pub(super) async fn prepare(
    ...
    if let Some(id_or_name) = cli.fork_session_id.as_deref() {
        let prepared = if should_load_configured_environments(&loader_overrides, target) {
            EnvironmentManager::prepare_from_antex_home(&source.codex_home).await
        } else {
    ...
    if !source.features.enabled(antex_features::Feature::Worktrees) {
    ...
    let manager = antex_worktree::WorktreeManager::new(
        antex_worktree::WorktreeSettings::for_cli(
            &source.codex_home,
