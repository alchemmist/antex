...
use super::antex_linux_sandbox_exe;
...
async fn wslg_duplicate_view_is_masked_with_and_without_fresh_procfs() {
    ...
    for no_proc in [false, true] {
        let mut command = tokio::process::Command::new(antex_linux_sandbox_exe());
        command
    ...
    let output = tokio::process::Command::new(antex_linux_sandbox_exe())
        .arg("--sandbox-policy-cwd")
        .arg(workspace.path())
        .args([
            "--permission-profile",
