...
impl From<&antex_protocol::openai_models::ModelInfo> for GuardianRejectionCircuitBreakerPolicy {
    fn from(model: &antex_protocol::openai_models::ModelInfo) -> Self {
        if model.model_specialty.as_deref()
            == Some(antex_protocol::openai_models::MODEL_SPECIALTY_CYBER)
        {
            Self::CyberModel
        } else {
            Self::Standard
