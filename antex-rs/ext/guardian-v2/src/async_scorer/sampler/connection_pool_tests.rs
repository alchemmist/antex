...
async fn cold_pool_uses_http_during_open_timeout_then_recovers_after_cooldown() -> Result<()> {
    ...
    for uses_antex_backend in [false, true] {
        ...
        let base_path = if uses_antex_backend {
            "/backend-api/codex"
        } else {
            "/v1"
        };
        ...
        config.provider = create_model_provider(
            ...
            Some(AuthManager::from_auth_for_testing(if uses_antex_backend {
        ...
        let expected_path = if uses_antex_backend {
        ...
        assert_eq!(
            ...
            uses_antex_backend.then_some("classifier")
        );
        ...
        assert_eq!(
            ...
            uses_antex_backend.then_some("resp-parent")
        );
