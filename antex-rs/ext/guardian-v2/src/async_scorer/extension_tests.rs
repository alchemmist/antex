...
) -> antex_config::GuardianPolicyLoader {
    antex_config::GuardianPolicyLoader::new(
        Some(&antex_features::FeatureToml::Config(
        ...
        &antex_config::ConfigRequirements::default(),
    )
}
...
impl RecordingMetrics {
    fn classification_samples(&self) -> Vec<RecordedMetric> {
        self.0.lock().unwrap().iter().filter(|sample| {
            !matches!(sample, RecordedMetric::Histogram(name, _, _) if name == antex_guardian_context::SECTION_COST_METRIC || name == antex_guardian_context::REQUEST_TOKENS_METRIC)
        }).cloned().collect()
    }
}
...
struct TestRetainedHistory {
    ...
    retained_context: Option<antex_history::RetainedContext>,
}
...
"#;
    ...
    assert!(metrics.0.lock().unwrap().iter().any(|sample| {
        ...
        if name == antex_guardian_context::SECTION_COST_METRIC
            && *value > 0
            && tags == &[
...
async fn contributor_skips_required_models_in_standard_scope() -> Result<()> {
    ...
    let _ = antex_core::guardian_review::prepare_review_prewarm(&test.antex).await?;
    let authorization = ScoreAuthorization::current(&test.antex).await;
    let progress = thread_store
        .get::<GuardianV2ScoreProgress>()
        .expect("Guardian v2 should track score progress per thread");
    *progress.authorization.lock().unwrap() = Some(authorization);
...
async fn assert_compaction_approval_policy(thread_context_enabled: bool) -> Result<()> {
    ...
    let score_authorization = ScoreAuthorization::current(&fixture.test.antex).await;
    *thread_store
        .get::<GuardianV2ScoreProgress>()
        .expect("score progress")
        .authorization
...
async fn assert_parent_compaction_reuse(thread_context_enabled: bool) -> Result<()> {
    ...
    let conversation_history = TestRetainedHistory {
        ...
        retained_context: thread_context_enabled.then(antex_history::RetainedContext::default),
    };
    thread_store.insert(parent_model);
...
struct CacheMiss;
impl antex_extension_api::SynchronousApprovalReviewer for CacheMiss {
    fn review(
        &self,
        _reason: antex_protocol::approvals::GuardianReviewReason,
    ) -> antex_extension_api::ExtensionFuture<'_, Option<ReviewDecision>> {
...
async fn cached_approval_discounts_only_its_own_unscored_wrapper() -> Result<()> {
    skip_if_no_network!(Ok(()));
    let fixture = GuardianFailureFixture::new().await?;
    let store = fixture.test.antex.thread_extension_data();
...
async fn cached_approval(
    registry: &antex_extension_api::ExtensionRegistry<Config>,
    store: &ExtensionData,
    action: &str,
    ...
    let category = match review_scope(&action) {
        ...
        None if store
            ...
            .other_tools
            == antex_protocol::openai_models::GuardianReviewMode::Adaptive =>
        {
            antex_protocol::openai_models::GuardianScope::Shell
        }
        None => {
    ...
    let input = antex_extension_api::ApprovalDecisionInput {
        ...
        action: &action,
        thread_id: antex_protocol::ThreadId::from_string(store.level_id()).unwrap(),
        thread_store: store,
        category,
    ...
    match registry.decide_approval(&input).await {
        Some(antex_extension_api::ApprovalDecision::Allow) => Some(ReviewDecision::Approved),
        _ => None,
    }
}
