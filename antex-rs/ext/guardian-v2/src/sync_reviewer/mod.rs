...
use antex_core::CodexResponsesHeaders;
...
use antex_core::guardian_review::GuardianReviewSession;
use antex_extension_api::ExtensionFuture;
use antex_extension_api::ExtensionRegistryBuilder;
use antex_extension_api::SessionIsolation;
...
use antex_guardian_reviewer::ReviewDenials;
use antex_guardian_reviewer::ReviewerPool;
use antex_guardian_reviewer::ReviewerTasks;
...
impl ThreadLifecycleContributor<Config> for GuardianExtension {
    fn on_thread_start<'a>(
        ...
        Box::pin(async move {
            ...
            input
                .thread_store
                .insert(antex_guardian_reviewer::ReviewerConfig::<Config>(
                ...
                ReviewerPool::<GuardianReviewSession>::new(
                    ...
                    move |context, key, kind, snapshot, cancel| {
                        ...
                        Box::pin(async move {
                            ...
                            if matches!(
                                kind,
                                antex_analytics::GuardianReviewSessionKind::EphemeralForked
                            ) {
                            ...
                            let provider = antex_model_provider::create_model_provider(
                            ...
                            options
                                .thread_extension_init
                                .insert(antex_guardian_reviewer::reviewer_allowed_tools());
    ...
    fn on_thread_ready<'a>(
        ...
        Box::pin(async move {
            ...
            tokio::spawn(async move {
                ...
                let prepare = async {
                    let context =
                        antex_core::guardian_review::prepare_review_prewarm(&parent).await?;
