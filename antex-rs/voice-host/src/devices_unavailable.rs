...
impl Devices {
    ...
    pub(super) fn take_state(&self) -> io::Result<antex_realtime_webrtc::AudioState> {
        Self::open().map(|_| antex_realtime_webrtc::AudioState::default())
    }
    ...
    pub(super) fn set_controls(&self, _: antex_realtime_webrtc::AudioControls) -> io::Result<()> {
        Self::open().map(|_| ())
    }
