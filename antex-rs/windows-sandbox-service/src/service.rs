...
pub(crate) fn run() -> Result<()> {
    let service_name = antex_windows_sandbox::windows_sandbox_service_name()?;
    let state = ServiceState {
        service_name: service_name.clone(),
        pipe_name: antex_windows_sandbox::windows_sandbox_service_pipe_name()?,
...
pub(crate) fn run_foreground() -> Result<()> {
    ...
    crate::ipc::run(
        ...
        || {
            eprintln!(
                "{} listening on {}",
                antex_windows_sandbox::windows_sandbox_service_name()?,
                antex_windows_sandbox::windows_sandbox_service_pipe_name()?
            );
            Ok(())
        },
