...
pub(crate) fn authorize_client_process(pipe: HANDLE) -> Result<AuthorizedClientProcess> {
    ...
    let client_family = unsafe { antex_windows_sandbox::process_package_family(process.0) }
        .context("read provisioning client package identity")?;
    let service_family =
        unsafe { antex_windows_sandbox::process_package_family(Threading::GetCurrentProcess()) }
...
pub(crate) fn authorize_client(
    ...
    let process_user = unsafe { antex_windows_sandbox::get_user_sid_bytes(process_token.0) }
        .context("read the client process user")?;
    let impersonated_user = unsafe { antex_windows_sandbox::get_user_sid_bytes(client_token) }
