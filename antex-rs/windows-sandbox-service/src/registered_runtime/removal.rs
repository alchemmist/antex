...
pub(crate) fn prepare(owner_token: HANDLE, record: &InstallationRecord) -> Result<PreparedRemoval> {
    ...
    for account in &record.runtime()?.accounts {
        if antex_windows_sandbox::local_user_flags(account.account.username())?.is_none() {
            ensure!(
        ...
        let token = with_owner_impersonation(owner_token, || {
            ...
            crate::ipc::pin_existing_ancestors(
                &antex_windows_sandbox::sandbox_secrets_dir(&record.codex_home),
                &mut pins,
            )?;
            antex_windows_sandbox::logon_existing_sandbox_account(
                &record.codex_home,
                account.account,
            )
        })?;
    ...
    let group_sid = if tokens.is_empty() {
        ...
        Some(
            antex_windows_sandbox::string_from_sid_bytes(&antex_windows_sandbox::resolve_sid(
                antex_windows_sandbox::SANDBOX_USERS_GROUP,
            )?)
    ...
    let plan = serde_json::json!({
        ...
        "record": record,
        "targets": targets,
        "service_name": antex_windows_sandbox::windows_sandbox_service_name()?,
        "group_sid": group_sid,
    });
