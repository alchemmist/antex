...
use antex_windows_sandbox::APP_CORE_RUNNER_ALIAS;
use antex_windows_sandbox::SandboxRuntimeAccount;
use antex_windows_sandbox::logon_existing_sandbox_account;
use antex_windows_sandbox::resolve_sid as lookup_sid;
...
use antex_windows_sandbox::token_groups;
use anyhow::Context;
use anyhow::Result;
use anyhow::ensure;
...
pub(crate) fn prepare_cleanup(mut record: InstallationRecord) -> Result<InstallationRecord> {
    ...
    for account in &record.runtime()?.accounts {
        if antex_windows_sandbox::local_user_flags(account.account.username())?.is_some() {
...
fn validate_target(
    ...
    let user = unsafe { antex_windows_sandbox::get_user_sid_bytes(token) }?;
    ...
    let sandbox_group = lookup_sid(antex_windows_sandbox::SANDBOX_USERS_GROUP)?;
