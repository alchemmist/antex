...
pub(super) fn run(
    ...
    on_authenticated_user: &dyn Fn(
        InstallationRecord,
        OwnedHandle,
        antex_windows_sandbox::SetupRuntime,
    ...
    let _setup_lock = antex_windows_sandbox::acquire_sandbox_setup_lock(/*timeout_ms*/ 5_000)?;
        ...
        antex_windows_sandbox::resolve_sid(antex_windows_sandbox::SANDBOX_USERS_GROUP)
            .is_ok_and(|current| current == sandbox_sid),
        antex_windows_sandbox::SANDBOX_GROUP_CHANGED
    );
    ...
    if request.refresh_only {
        ...
        for entry in &runtime.accounts {
            let account = entry.account.username();
                ...
                antex_windows_sandbox::local_user_flags(account)?
                    .is_some_and(|flags| flags & UF_ACCOUNTDISABLE == 0)
                    && string_from_sid_bytes(&antex_windows_sandbox::resolve_sid(account)?)
    ...
    let result = (|| -> Result<()> {
        ...
        for account in [
            ...
            setup_complete &= antex_windows_sandbox::local_user_flags(account)?
                .is_some_and(|flags| flags & UF_ACCOUNTDISABLE == 0);
        }
        if !setup_complete {
            ...
            let setup_owner_sid = antex_windows_sandbox::resolve_sid(&identity.account)?;
            ensure!(
            ...
            antex_windows_sandbox::provision_sandbox_in_process(
                &identity.codex_home,
                &identity.account,
                settings,
                identity.runtime,
