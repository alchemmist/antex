...
pub(super) fn foreground_owner(
    ...
    runtime: antex_windows_sandbox::SetupRuntime,
) -> Result<crate::installation_record::InstallationRecord> {
    let _lock = antex_windows_sandbox::acquire_sandbox_setup_lock(/*timeout_ms*/ 5_000)?;
    if let Some(previous) = crate::installation_record::load()? {
        ...
        } else if runtime == antex_windows_sandbox::SetupRuntime::Registered {
            anyhow::ensure!(
                same_owner,
