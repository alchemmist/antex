...
use antex_windows_sandbox::SetupRuntime;
...
fn authenticate_impersonated_client(
    ...
    let user = unsafe { antex_windows_sandbox::get_user_sid_bytes(token.0) }
        .context("read provisioning client identity")?;
    let user_sid = string_from_sid_bytes(&user).map_err(anyhow::Error::msg)?;
    let account = unsafe { antex_windows_sandbox::account_name_from_sid(user.as_ptr() as _) }
    ...
    let (codex_home, handles) = match request {
        ServiceRequest::ProvisionSandbox(request) => prepare_antex_home(
            requested_home,
            runtime,
        ...
        ServiceRequest::RegisterInstallation { .. } => {
            ...
            antex_windows_sandbox::validate_local_directory_path(requested_home)?;
